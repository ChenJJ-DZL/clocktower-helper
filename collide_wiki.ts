...
for (const r of wikiRoles) {
  const ability = abilities.find(
    (a: any) =>
      norm(a.roleId) === norm(r.wikiName) ||
      (r.enName && norm(a.roleId) === norm(r.enName))
  );
  if (!ability) continue;
...
console.log(
  `对撞点总数: ${collisions.length}（国风 ${gf.length} / 官方+实验 ${official.length}）\n`
);
...
report.push(
  `生成时间: 2026-08-15 | 对撞点: ${collisions.length}（国风 ${gf.length} / 官方+实验 ${official.length}）\n`
);
report.push(
...
for (const c of collisions) {
  ...
  report.push(
    `- **本地实现**: ${c.localAbilityName} | timing=[${c.localTimings.join(",")}] fn=${c.localFn} on=${c.localOn} target=${JSON.stringify(c.localTarget)}`
  );
  report.push("");
}
fs2.writeFileSync(
  path.join(__dirname, "json", "wiki_crawl", "对撞报告.md"),
  report.join("\n"),
  "utf-8"
);
...
for (const c of gf) {
  ...
  console.log(
    `  wiki规则: ${c.wikiAbility.slice(0, 90)}${c.wikiAbility.length > 90 ? "…" : ""}`
  );
  console.log(
    `  本地实现: ${c.localAbilityName} | timing=[${c.localTimings.join(",")}] fn=${c.localFn} on=${c.localOn} target=${JSON.stringify(c.localTarget)}`
  );
