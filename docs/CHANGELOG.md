...
### 一、伪装身份面板重构与体验优化
1. **默认卡片格式展示与强制单行**：
   - 面板默认以卡片形式展示场上需要设置伪装的角色（提线木偶、酒鬼、疯子）；
   - 未设置时显示：`1号【角色名】 → 伪装【未设置】 设置 ✎`；
   - 已配置时显示：`1号【角色名】 → 伪装为【伪装角色名】 修改 ✎`；
   - 彻底省去多余的“手动选择伪装身份”按钮，保留全宽“🎲 一键随机分配 / 再次一键随机分配”；
   - 无论桌面端还是移动端、无论角色名字长短，全局注入强制样式（`white-space: nowrap !important; word-break: keep-all !important`），确保内容 100% 保持在单行内，彻底消除换行现象。
2. **单角色独立点击设置与修改**：
   - 点击角色卡片项直接打开专属伪装弹窗（`🎭 为 X号【角色名】设定伪装身份`），可独立为此角色选择候选身份或单独随机；
   - 点击一键随机分配则为全场需要伪装的角色一键生成合法角色，重复点击可重新触发随机生成。
3. **开始游戏前强校验拦截与开局后自动隐藏**：
   - 若场上仍有未分配伪装的角色，点击“开始游戏”进行拦截，弹出 2.7 秒渐入渐出 UI Toast 提示：`请设置XX角色伪装`；
   - 正式开始游戏后自动隐藏伪装面板，并同步在玩家右键菜单中锁定伪装修改。

### 二、手机端浏览器点击剧本后崩溃/拦截根治
1. **解除 PortraitLock 全屏遮罩阻断**：
   - 移除全屏阻断遮罩，消除 iOS WebKit `backdrop-blur` 导致的 GPU/内存溢出与进程闪退问题，彻底释放移动端触摸和点击事件；
2. **ScaleLayout 移动端全屏等比自适应**：
   - 移除 `Math.max(0.35, ...)` 硬编码截断，改为 `Math.min(scaleX, scaleY)`，使 1600x900 魔典画布按手机屏幕宽度自适应等比缩放；
   - 彻底解决手机端控制台被挤出视口裁切的问题，圆桌、顶部栏、右侧控制台与按钮全部完整位于屏幕内；
   - 手机端无需手动勾选“请求桌面网页”，直接打开即可顺畅进入具体剧本并完整运行游戏。
