...
test.describe("L4 状态断言 · 罂粟花开：真实点击流必须产生真实状态变更", () => {
  ...
  test("⭐ 第二夜恶魔行动后：凡死亡/待死者必须落成状态字段（isDead + 死因）", async ({
    ...
    // ── 进入第二夜：黄昏 → 下一夜 ──
    /**
     * 🔴🔴 2026-09-22 自我纠错：**原 skip 是错的**，理由如下（当时被记为「已知自动化能力边界」）。
     *
     * 原记录（2026-09-21 探针）说：「失败局的特点是洗脑师存活，白天出现日间门禁
     *   `需先完成洗脑师判定【疯狂洗脑】`；点『疯狂洗脑』后**页面无任何反应**
     *   （文本长度 11976→11976 不变、『确定使用』未出现、dialogs=0）⇒ 无法自动解除」。
     *
     * ❌ **实测推翻了这个结论**（2026-09-22 探针，罂粟花开 与 梦殒春宵**逐步骤完全镜像**）：
     *   点「疯狂洗脑」后弹窗**确实出现**：
     *     `[step2] dlg="使用技能✕确定使用疯狂洗脑吗？取消确认"`
     *     `[step3] dlg="🧠疯狂洗脑判定…[取消][是(通过/无事发生)][否(处决并跳入下一夜)]"`
     *   ⇒ 点「确认」→ 点「是(通过/无事发生)」→ `gated=false`、「进入黄昏处决阶段」可点 ✅
     *
     * 🔎 **真根因在 E2E helper 自己身上**：`completeDuskEnterNight` 的门禁弹窗判据是
     *   `/^否|无事发生|^取消/` —— 「使用技能」确认框里**有「取消」按钮**
     *   ⇒ helper 每次都点**取消**、把技能取消掉 ⇒ 门禁永不解除 ⇒ 超时
     *   ⇒ 再被误读成「按钮点了没反应」。
     * ✅ 已修：判据收紧为 `/无事发生/`（精确命中三类弹窗的**保守项**，
     *   且**不会**命中 `取消` 或 `否(处决并跳入下一夜)`）。
     *
     * ⇒ **生产无缺陷**。本用例恢复为**硬断言**（再失败就是新问题，必须吵）。
     */
