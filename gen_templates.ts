...
const allChars: Array<{ id: string; 名称: string; 英文名: string; 类型: string; 所属剧本: string; content?: { 角色能力?: string } }> = [];
for (const f of fs.readdirSync(path.join(__dirname, "json", "full"))) {
  if (!f.endsWith(".json")) continue;
  const d = JSON.parse(fs.readFileSync(path.join(__dirname, "json", "full", f), "utf-8"));
  for (const c of (Array.isArray(d) ? d : [])) allChars.push(c);
}

// 英文名小写(去下划线/空格/撇号) → 角色数据
const byEnglish = new Map<string, { 名称: string; 能力: string; 剧本: string }>();
for (const c of allChars) {
  if (!c.英文名) continue;
  const key = c.英文名.toLowerCase().replace(/[^a-z0-9\u4e00-\u9fa5]/g, "");
  if (!byEnglish.has(key)) {
    byEnglish.set(key, { 名称: c.名称, 能力: c.content?.["角色能力"] ?? "", 剧本: c.所属剧本 ?? "" });
  }
}
...
function pushTemplate(tplId: string, roleName: string, desc: string, source: string) {
...
for (const a of abilities as any[]) {
  ...
  pushTemplate(`role.${a.roleId}.wake`, roleName, desc, doc?.剧本 ?? "官方文档");

  // 2. 实际 wakePromptId（若非规范 id 且非 default/空）
  if (promptId && promptId !== `role.${a.roleId}.wake` && promptId !== "default_wake") {
    pushTemplate(promptId, roleName, desc, doc?.剧本 ?? "官方文档");
  }
}

console.log(`// 自动生成：${lines.length} 条（官方文档 ${matched} / 兜底 ${fallback}，跳过已有 ${already}）`);
