...
async function runNight(
  snap: any,
  pickTargets?: (node: any, snapshot: any, ability: any) => number[]
) {
...
async function main() {
  console.log("===== 穷奇：活尸机制 =====");
  {
    ...
    const r = await runNight(snap, (node: any) =>
    ...
    const dead = r.finalSnapshot.seats
      ...
      .map((s: any) => s.id);
    check(
      "穷奇目标进入活尸（isDead=true）",
      target?.isDead === true,
      JSON.stringify({ isDead: target?.isDead, effects: target?.statusEffects })
    );
    check(
      "活尸带 alive_dead 标记",
      target?.statusEffects?.some((e: any) => e.type === "alive_dead") === true
    );
  ...
  }
  {
    ...
    check(
      "穷奇正常击杀（无活尸标记）",
      target?.isDead === true &&
        !target?.statusEffects?.some((e: any) => e.type === "alive_dead"),
      `effects=${JSON.stringify(target?.statusEffects)}`
    );
  }
  ...
  console.log("===== 饕餮：类型互异判定 =====");
  {
    ...
    const deadIds = r.finalSnapshot.seats
      .filter((s: any) => s.isDead)
      .map((s: any) => s.id)
      .sort();
    check(
  ...
  console.log("===== 梼杌：替死机制（imp 杀梼杌）=====");
  {
    ...
    check(
      ...
      minion?.statusEffects?.some((e: any) => e.type === "lost_ability") ===
        true,
      `effects=${JSON.stringify(minion?.statusEffects)}`
    );
  }
  {
    // 无爪牙可替死 → 梼杌死亡
  ...
  console.log("===== 鸩：限一次毒杀镇民角色 =====");
  {
    ...
    const r = await runNight(snap, (node: any, s: any, ab: any) =>
      node.roleId === "zhen" ? [] : []
    );
  ...
  // 手动定向执行鸩（带 storytellerInput）
  {
    ...
    check(
      "鸩毒杀在场镇民角色（中毒+死亡）",
      victim?.isDead === true &&
        victim?.statusEffects?.some((e: any) => e.type === "poisoned") === true,
      `isDead=${victim?.isDead} effects=${JSON.stringify(victim?.statusEffects)}`
    );
    ...
    check(
      "鸩第二次使用被限次拦截",
      out2.aborted === true,
      `aborted=${out2.aborted} reason=${out2.abortReason}`
    );
