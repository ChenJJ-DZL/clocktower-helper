...
} from "./src/utils/morticianTransform";
...
async function main() {
  ...
  console.log("\n── 知府 ──");
  {
    ...
    check(
      "A: 当晚有爪牙死亡 → 得知'是'",
      action?.context.meta?.abilityResult?.hasNonTownsfolkDeath === true
    );
    check(
      "A2: 结算弹窗为'是'",
      action?.context.meta?.displayInfo?.message === "是"
    );

    // B: 当晚无死亡 → 得知"否"（仍被唤醒）
    const seatsB = seats.map((s) => ({
      ...s,
      isAlive: true,
      isDead: false,
    ...
    check(
      ...
      !!actionB &&
        !actionB.aborted &&
        actionB.context.meta?.abilityResult?.hasNonTownsfolkDeath === false
    );
  }
  ...
  console.log("\n── 酿酒师 ──");
  {
    ...
    const r = await runNight(
      ...
      2,
      {},
      { targetRoleId: "washerwoman", message: "酿酒师的假信息" }
    );
    const brewerAction = r.actions.find((a) => a.node.roleId === "brewer");
    check(
      "A: 酿酒师设置成功",
      !!brewerAction &&
        !brewerAction.aborted &&
        brewerAction.context.snapshot?.brewerEffect?.roleId === "washerwoman"
    );
    check(
      "A2: 结算产物含目标角色与信息",
      brewerAction?.context.meta?.displayInfo?.targetRoleId === "washerwoman"
    );
    ...
    check(
      "B: 目标角色信息被替换",
      ww?.context.meta?.brewerOverride === "替换后的信息"
    );
    check(
      "B2: brewerEffect 已被消耗",
      rB.finalSnapshot.brewerEffect === undefined
    );
  }
  ...
  console.log("\n── 提刑官 ──");
  {
    ...
    check(
      "B: 未提名当晚不唤醒",
      actB?.aborted === true && !!actB?.abortReason?.includes("未发起提名")
    );
  }
  ...
  console.log("\n── 引路人 ──");
  {
    ...
    check(
      "A: 选中的 1 号被恶魔选为目标 → '是'",
      act?.context.meta?.abilityResult?.isYes === true
    );

    // B: 所选玩家未被命中 → "否"
    const rB = await runNight(
      seats,
      2,
      { nightEvilTargets: [2] },
      undefined,
    ...
    check(
      "B: 选中的 4 号未被命中 → '否'",
      actB?.context.meta?.abilityResult?.isYes === false
    );
  }
  ...
  console.log("\n── 掮客 ──");
  {
    ...
    check(
      "A: 掮客识别同阵营",
      brokerAct?.context.meta?.abilityResult?.swapActive === true
    );
    // 重定向生效：imp 执行内部（calculate 后）目标变为 3
    check(
      "A2: 恶魔目标被重定向 4→3",
      impAct?.context.meta?.abilityResult?.targetId === 3
    );
  }
  ...
  console.log("\n── 入殓师 ──");
  {
    ...
    check(
      "A2: 入殓师变为恶魔",
      afterA.find((s) => s.id === 0)?.role?.id === "imp" &&
        afterA.find((s) => s.id === 0)?.role?.type === "demon"
    );
    ...
    check(
      "B: 存活(旅行者除外)=3 <4 → 不转化",
      rB.transformed === false && rB.reason.includes("失去能力"),
      rB.reason
    );
    ...
    check(
      "C: 非恶魔被处决不转化",
      rC.transformed === false && rC.reason.includes("不是恶魔")
    );
  ...
  console.log("\n── 戏子 ──");
  {
    ...
    check(
      ...
      !!act &&
        !act.aborted &&
        Array.isArray(res?.actors) &&
        Array.isArray(res?.evilPlayers)
    );
    check(
      "A2: 邪恶玩家含 1、2 号",
      res?.evilPlayers?.includes(1) === true &&
        res?.evilPlayers?.includes(2) === true
    );

    // B: 判胜对调
    check(
      "B: 有戏子 → 善良胜对调为邪恶胜",
      applyActorVictoryFlip("good", seats) === "evil"
    );
    check(
      "B2: 无戏子 → 不调换",
      applyActorVictoryFlip(
        "good",
        seats.map((s) => ({ ...s, role: { ...s.role, id: "imp" } }))
      ) === "good"
    );
    ...
    check(
      "C: 所有善良玩家变戏子",
      afterSetup.filter((s) => s.role?.id === "actor").length === 3
    );
    check(
      "C2: 恶魔保持身份",
      afterSetup.find((s) => s.id === 1)?.role?.id === "imp"
    );
  }
