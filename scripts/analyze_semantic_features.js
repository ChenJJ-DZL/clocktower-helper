...
const SEMANTIC_CATEGORIES = {
  FAKE_IDENTITY: {
    name: '假象/欺骗/双重身份类',
    keywords: ['fakeRole', 'apparentRole', 'charadeRole', 'drunk', 'lunatic', 'marionette', 'spy', 'widow', 'damsel', 'recluse', 'poisoner'],
    description: '角色具有假象身份或欺骗机制'
  },
  MULTI_CANDIDATE: {
    name: '多候选/模糊信息生成类',
    keywords: ['candidate', 'washerwoman', 'librarian', 'investigator', 'dreamer', 'savant', 'juggler', 'gossip', 'fisherman', 'oracle'],
    description: '角色提供模糊或多个候选信息'
  },
  DAY_INSTANT: {
    name: '白天即时结算/公开决斗类',
    keywords: ['psychopath', 'slayer', 'virgin', 'golem', 'witch', 'gunslinger', 'nominate', 'execute'],
    description: '白天即时触发的能力'
  },
  FACTION_SWAP: {
    name: '阵营互换/身份跃迁类',
    keywords: ['snake_charmer', 'fang_gu', 'pit_hag', 'barber', 'klutz', 'goon', 'politician', 'ogre'],
    description: '角色可以改变阵营或身份'
  },
  DELAYED_EFFECT: {
    name: '延迟生效/非标准夜行动力学',
    keywords: ['pukka', 'po', 'sailor', 'ojo', 'al_hadikhia', 'shabaloth', 'zombuul', 'exorcist'],
    description: '延迟或非标准的夜间行动机制'
  },
  GLOBAL_RULE: {
    name: '全局规则接管/特殊终局',
    keywords: ['heretic', 'evil_twin', 'saint', 'goblin', 'mastermind', 'leviathan', 'riot', 'legion', 'vizier', 'atheist'],
...
function extractSemanticFeatures(content, roleId) {
  ...
  // 检查假象身份机制
  if (content.includes('charadeRole') || content.includes('fakeRole') || content.includes('apparentRole')) {
    features.push('FAKE_IDENTITY');
  }
  
  // 检查醉酒/中毒相关
  if (content.includes('isDrunk') || content.includes('isPoisoned') || content.includes('abilityEffective')) {
    features.push('FAKE_IDENTITY');
  }
  
  // 检查多候选信息
  if (content.includes('candidate') || content.includes('randomize') || content.includes('shuffle')) {
    features.push('MULTI_CANDIDATE');
  }
  
  // 检查白天即时结算
  if (content.includes('nominate') || content.includes('execute') || content.includes('dayAction')) {
    features.push('DAY_INSTANT');
  }
  
  // 检查阵营互换
  if (content.includes('swap') || content.includes('convert') || content.includes('alignment')) {
    features.push('FACTION_SWAP');
  }
  
  // 检查延迟生效
  if (content.includes('delay') || content.includes('deferred') || content.includes('nextNight')) {
    features.push('DELAYED_EFFECT');
  }
  
  // 检查全局规则
  if (content.includes('winCondition') || content.includes('gameOver') || content.includes('globalRule')) {
...
function analyzeRoleCompleteness(roleId, abilityContent, definitionContent) {
  ...
  // 检查是否有简单的选人逻辑但缺少复杂语义
  if (abilityContent && abilityContent.includes('selectPlayer') && !abilityContent.includes('calculate')) {
    gaps.push('仅有简单选人逻辑');
  }
...
async function analyzeSemanticFeatures() {
  ...
  if (fs.existsSync(NEW_ENGINE_DIR)) {
    const abilityFiles = fs.readdirSync(NEW_ENGINE_DIR).filter(f => f.endsWith('.ability.ts'));
    ...
    for (const file of abilityFiles) {
      ...
      const definitionContent = scanFile(path.join(ROLES_DIR, '**', `${roleId}.ts`));
      
      results.rolesWithAbilities++;
      
      // 分析完整性
      const gaps = analyzeRoleCompleteness(roleId, abilityContent, definitionContent);
      if (gaps.length > 0) {
  ...
  for (const [key, category] of Object.entries(results.semanticCategories)) {
    ...
    if (category.roles.length > 0) {
      console.log(`  角色: ${category.roles.slice(0, 10).join(', ')}${category.roles.length > 10 ? '...' : ''}`);
    }
  }
  ...
  const emptyRoles = results.gapAnalysis.filter(g => g.gaps.includes('仅有简单选人逻辑'));
  if (emptyRoles.length > 0) {
    console.log(`\n1. 需要增强语义的角色 (${emptyRoles.length} 个):`);
    emptyRoles.forEach(r => console.log(`   - ${r.roleId}`));
  }
  
  const missingStateUpdate = results.gapAnalysis.filter(g => g.gaps.includes('缺少状态更新逻辑'));
