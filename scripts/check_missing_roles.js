...
while ((match = roleIdsSectionRegex.exec(tempContent)) !== null) {
  ...
  if (ids) {
    ids.forEach(id => {
      ...
      // 确保不是剧本ID（剧本ID都比较长）
      if (!['trouble_brewing', 'bad_moon_rising', 'sects_and_violets', 
           'whispering_secrets', 'tomb_of_the_unknown', 'high_pleasure', 
           'haunted_manor', 'garden_of_dreams'].includes(cleanId)) {
