            ...
            if table:
                for row in table.find_all('tr')[1:]:  # 跳过表头
                    ...
                    if len(cols) >= 2:
                        ...
                        order_data.append({"step": step, "description": description})
    return order_data
...
def collect_rules():
    ...
    for section in sections:
        ...
        if content:
            # 处理特殊文件名
            filename = section.replace(" ", "_").replace("/", "-") + ".json"
            filepath = os.path.join(RULES_DIR, filename)
            
            with open(filepath, 'w', encoding='utf-8') as f:
                json.dump({"title": section, "content": content}, f, ensure_ascii=False, indent=2)
            print(f"已保存: {filename}")
    
    # 特别处理夜晚行动顺序
    first_night = extract_night_order(soup, "夜晚行动顺序一览（首夜）")
    other_nights = extract_night_order(soup, "夜晚行动顺序一览（其他夜晚）")
    
    # 保存夜晚行动顺序
    with open(os.path.join(RULES_DIR, "夜晚行动顺序一览（首夜）.json"), 'w', encoding='utf-8') as f:
        json.dump(first_night, f, ensure_ascii=False, indent=2)
    
    with open(os.path.join(RULES_DIR, "夜晚行动顺序一览（其他夜晚）.json"), 'w', encoding='utf-8') as f:
        json.dump(other_nights, f, ensure_ascii=False, indent=2)
