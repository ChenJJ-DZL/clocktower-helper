...
for (const file of files) {
    if (file.endsWith('.json')) {
        try {
            const data = JSON.parse(fs.readFileSync(path.join(josnDir, file), 'utf8'));
...
function parseContent(content) {
    ...
    if (rulesMatch) {
        ...
        detailedDescription = content.replace(/【规则细节】\n[\s\S]*?(?=\n【|$)/, '').trim();
    }
    ...
    for (const entry of entries) {
        ...
        if (fs.statSync(fullPath).isDirectory()) {
            ...
            if (roleDocs.has(roleName)) {
                ...
                const { detailedDescription, clarifications } = parseContent(docContent);
                ...
                if (typeMatch) {
                    ...
                    // Format strings:
                    const safeDesc = '`' + detailedDescription.replace(/`/g, '\\`').replace(/\${/g, '\\${') + '`';

                    let injection = `\n  detailedDescription: ${safeDesc},`;

                    if (clarifications.length > 0) {
                        const safeClar = clarifications.map(c => '`' + c.replace(/`/g, '\\`').replace(/\${/g, '\\${') + '`').join(',\n    ');
                        injection += `\n  clarifications: [\n    ${safeClar}\n  ],`;
                    }
