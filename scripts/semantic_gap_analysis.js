...
// 加载数据
const rolesData = JSON.parse(fs.readFileSync(path.join(BASE, 'src/data/rolesData.json'), 'utf-8'));
const officialDocs = JSON.parse(fs.readFileSync(path.join(BASE, 'src/data/officialRoleDocs.json'), 'utf-8'));
const nightOrder = JSON.parse(fs.readFileSync(path.join(BASE, 'src/data/nightOrder.json'), 'utf-8'));
...
const rolesWithoutNightMeta = rolesData.filter(r => 
  !r.firstNightMeta && !r.otherNightMeta && !r.dayMeta && r.type !== 'traveler'
);

console.log(`\n⚠️ 缺少夜晚/白天元数据的角色 (${rolesWithoutNightMeta.length}):`);
rolesWithoutNightMeta.slice(0, 20).forEach(r => console.log(`  - ${r.id} (${r.name})`));
...
rolesWithoutEffectType.slice(0, 10).forEach(r => console.log(`  - ${r.id} (${r.name})`));
