...
function formatDuration(duration: string): string {
  ...
  if (duration.includes('至下个黄昏') || duration.includes('下个黄昏清除') || duration.includes('次日黄昏清除')) return '至黄昏';
  if (duration.includes('至天亮') || duration.includes('至下个白天') || duration === 'until_dawn') return '至天亮';
...
export const SeatNode: React.FC<SeatNodeProps> = (props) => {
  ...
  const {
    ...
    s, i, seats, isPortrait, seatScale, nightInfo, selectedActionTargets,
    getSeatPosition, getDisplayRoleType, typeColors
  );
      ...
      >
        ...
        <div className={`absolute ${isPortrait ? '-top-1.5 -right-1.5' : '-top-5 -right-5'} flex flex-col gap-0.5 items-end z-40`}>
          {seats.some((seat) => seat.masterId === s.id) && (
            <span className={`${isPortrait ? 'text-[7px] px-0.5 py-0.5' : 'text-xs px-2 py-0.5'} bg-purple-600 rounded-full shadow font-bold`}>主人</span>
          )}
          {s.isCandidate && (
        ...
        {s.isDead && s.hasGhostVote && (
          <div className={`absolute ${isPortrait ? '-bottom-1 -right-1' : '-bottom-1 -right-1'} ${isPortrait ? 'w-4 h-4' : 'w-5 h-5'} bg-white rounded-full border-2 border-slate-900 flex items-center justify-center shadow-md z-30`}>
