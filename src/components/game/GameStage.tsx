            ...
            <div className="h-full flex flex-col justify-between p-4 relative z-40 bg-slate-900/60 backdrop-blur-md">
              ...
              <div className="flex flex-col gap-2.5 overflow-y-auto pr-1 text-[14px] select-none">
                ...
                <div className="border-b border-white/10 pb-2 shrink-0 space-y-1">
                  ...
                  <div className="flex">
                    <span className="px-2.5 py-1 rounded-md bg-amber-500/15 text-amber-300 border border-amber-500/30 text-[0.92em] font-bold">
                ...
                <div className="space-y-1.5">
                  {pendingVoteFor !== null ? (
                        ...
                        <div className="flex items-center gap-2 text-slate-200">
                          ...
                          <span>
                            ...
                            <strong className="text-cyan-300">
                              {pendingVoteFor + 1}号
                              {seats[pendingVoteFor]?.role?.name
                                ? ` (${seats[pendingVoteFor].role.name})`
                                : ""}
                            </strong>{" "}
                            辩护
                    ...
                    <div className="p-3 rounded-lg bg-slate-800/80 border border-white/10 space-y-1.5 text-[1em]">
                      ...
                      <div className="text-[0.92em] text-gray-300">
                        点圆桌座位依次选【提名者】→【被提名者】
                      </div>
                    </div>
                  )}
                ...
                {/* 模块 3：说书人主持话术与黄昏规则指南（默认收起，避免挤占操作区） */}
              ...
              <div className="flex flex-col gap-2 pt-2.5 shrink-0 border-t border-white/10 relative z-50 mt-1">
                ...
                <button
                  ...
                  onClick={() => {
                    ...
                    const hasCandidates = seats.some(
                      (s: Seat) => s.isCandidate
                    );
                    if (hasPendingVote || hasCandidates) {
                      showConfirm({
                        title: "处决未结算",
                        message:
                          "当前仍有处决候选未结算。按照规则，进入下一夜的前提是完成处决结算，处决不可跳过。",
                        cancelLabel: "返回",
                        confirmLabel: "执行处决",
                        onConfirm: () => {
                          try {
  ...
  return (
    <>
      ...
      <GlobalPrivacyShield
        isActive={isPrivacyShieldActive}
        onDismiss={() => setIsPrivacyShieldActive(false)}
        isNightPhase={gamePhase === "firstNight" || gamePhase === "night"}
      />
    </>
  );
};
