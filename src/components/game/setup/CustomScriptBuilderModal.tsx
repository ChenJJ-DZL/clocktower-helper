...
import { Role, roles, groupedRoles, typeBgColors, typeLabels } from '../../../../app/data';
...
export function CustomScriptBuilderModal({ onClose, onSave }: CustomScriptBuilderModalProps) {
    ...
    const builderGroupedRoles = useMemo(() => {
        return builderRoles.reduce((acc, role) => {
            ...
            <div className="bg-slate-900 rounded-2xl w-full max-w-5xl h-full max-h-[90vh] flex flex-col shadow-2xl border border-slate-700 overflow-hidden animate-in fade-in zoom-in-95 duration-200">
                ...
                <div className="flex items-center justify-between p-6 border-b border-slate-700 bg-slate-800/50">
                    <div>
                        ...
                        <p className="text-sm text-slate-400 mt-1">从全部角色库中自由组合，DIY您的专属剧本</p>
                    </div>
                    <button
                        onClick={onClose}
                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-6">
                    <div className="space-y-2">
                        <label className="text-sm font-semibold text-slate-300">剧本名称 <span className="text-red-400">*</span></label>
                        <input
                            type="text"
                            value={scriptName}
                    ...
                    <div className="space-y-2">
                        ...
                        <div className="space-y-6">
                                ...
                                return (
                                    <div key={type} className="space-y-3">
                                        <div className="flex items-center gap-2 border-b border-slate-700/50 pb-2">
                                            <span className="text-lg font-bold text-slate-200">{typeLabels[type]}</span>
                                            <span className="text-xs text-slate-500 bg-slate-800 px-2 py-0.5 rounded-full">
                                                {Array.from(selectedRoleIds).filter(id => typeList.some(r => r.id === id)).length} / {typeList.length}
                                            </span>
                                        </div>
                            ...
                            `}
                                                    >
                                                        <span className={`text-sm font-bold whitespace-nowrap ${isSelected ? 'text-white' : 'text-slate-300'}`}>
                                                            {r.name}
                                                        </span>
