    ...
    <ModalWrapper
      ...
      footer={
        <>
          ...
          <button
            className="px-4 py-2 bg-red-600 rounded font-bold hover:bg-red-700 transition-colors"
            onClick={onConfirm}
          >
            确定
          </button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-4">
        {targetIds.map((id) => {
          ...
          return (
            <div
              key={id}
              className="bg-gray-900 border border-gray-700 rounded px-3 py-2 space-y-2"
            >
              <div className="flex items-center justify-between text-white font-bold">
                <span>
                  [{id + 1}] {seat?.role?.name || "未知"}
                </span>
                {seat?.isDead ? (
                  <span className="text-red-300 text-xs">已死</span>
                ) : (
                  <span className="text-green-300 text-xs">存活</span>
                )}
              </div>
