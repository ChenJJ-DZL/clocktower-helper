...
export function RangerModal({
  ...
  return (
    <ModalWrapper
      ...
      footer={
        <button
          className="px-4 py-2 bg-green-600 rounded font-bold hover:bg-green-700 transition-colors"
          onClick={onConfirm}
        >
          确定
        </button>
    ...
    >
      <div className="text-gray-200 mb-4">
        目标：{targetId + 1}号(落难少女) —
        必须为其选择当前剧本的镇民角色（已在场镇民不可选，不可取消）
      </div>
      <select
