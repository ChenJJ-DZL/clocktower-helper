...
export function SpyDisguiseModal() {
  ...
  const spyDisguiseModal =
    props.currentModal?.type === "SPY_DISGUISE" ? props.currentModal : null;
  ...
  const investigatorSeat = props.seats.find(
    (s: any) => s.role?.id === "investigator"
  );
  const fortuneTellerSeat = props.seats.find(
    (s: any) => s.role?.id === "fortune_teller"
  );
          ...
          <div className="space-y-3 text-sm">
            {spySeats.length > 0 && (
              <div>
                ...
                {spySeats.map((s: any) => (
                  <div key={s.id} className="text-gray-300 ml-2">
                    {s.id + 1}号
                  </div>
                ))}
              </div>
            ...
            {props.spyDisguiseMode === "on" && (
              <div className="flex items-center gap-2">
                ...
                <input
                  ...
                  onChange={(e) =>
                    props.setSpyDisguiseProbability(
            ...
            {props.spyDisguiseMode === "default" && (
              <div className="text-xs text-gray-400">默认概率：80%</div>
            )}
            {(chefSeat ||
              empathSeat ||
              investigatorSeat ||
              ...
              <div className="text-xs text-gray-400 pt-2 border-t border-gray-700">
                受影响角色：{chefSeat && "厨师"}{" "}
                {chefSeat &&
                  (empathSeat || investigatorSeat || fortuneTellerSeat) &&
                  "、"}
                {empathSeat && "共情者"}{" "}
                {(chefSeat || empathSeat) &&
                  (investigatorSeat || fortuneTellerSeat) &&
                  "、"}
                {investigatorSeat && "调查员"}{" "}
                {(chefSeat || empathSeat || investigatorSeat) &&
                  fortuneTellerSeat &&
                  "、"}
            ...
            {registrationInfo && (
              <div className="mt-3 border-t border-gray-700 pt-2 text-xs text-gray-300 space-y-2">
                ...
                {registrationInfo.affected.map((target: any) => (
                  <div
                    key={target.id}
                    className="bg-gray-750 rounded p-2 border border-gray-700"
                  >
                    <div className="font-medium mb-1">
                      {target.id + 1}号【{target.role?.name || "未知"}】
                    </div>
