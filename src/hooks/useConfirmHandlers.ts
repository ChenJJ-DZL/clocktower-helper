...
export interface ConfirmHandlersDeps {
    ...
    currentHint: { isPoisoned: boolean; guide: string; speak: string; fakeInspectionResult?: string | null };
    ...
    setHadesiaChoices: React.Dispatch<React.SetStateAction<Record<number, 'live' | 'die'>>>;
    ...
    executePlayer: (id: number, options?: { skipLunaticRps?: boolean; forceExecution?: boolean }) => void;
    saveGameRecord: (record: GameRecord) => void;

    // Sub-hook results
    nightLogic: { processDemonKill: (targetId: number, options?: any) => 'pending' | 'resolved' };
...
export function useConfirmHandlers(deps: ConfirmHandlersDeps) {
    ...
    }, [currentModal, seats, killPlayer, continueToNextAction, addLog, setCurrentModal, moonchildChainPendingRef]);
    ...
    const confirmSweetheartDrunk = useCallback((targetId: number) => {
        ...
        onResolve?.();
        continueToNextAction();
    }, [currentModal, setSeats, addDrunkMark, continueToNextAction, addLog, setCurrentModal]);
    ...
    }, [currentModal, klutzChoiceTarget, seats, isEvil, checkGameOver, setCurrentModal, setKlutzChoiceTarget, setWinResult, setWinReason, setGamePhase, addLog]);
    ...
    const confirmHadesia = useCallback(() => {
        if (!nightInfo || currentModal?.type !== 'HADESIA_KILL_CONFIRM') return;
        const baseTargets = currentModal.data.targetIds;
        const demonName = getDemonDisplayName(nightInfo.effectiveRole.id, nightInfo.effectiveRole.name);
        const choiceMap = baseTargets.reduce<Record<number, 'live' | 'die'>>((acc: Record<number, 'live' | 'die'>, id: number) => {
            acc[id] = hadesiaChoices[id] || 'live';
            return acc;
        }, {});

        const allChooseLive = baseTargets.every((id: number) => choiceMap[id] === 'live');
        const finalTargets = allChooseLive ? baseTargets : baseTargets.filter((id: number) => choiceMap[id] === 'die');

        const choiceDesc = baseTargets.map((id: number) => `[${id + 1}号${choiceMap[id] === 'die' ? '死' : '生'}]`).join('、');
        addLog(`${nightInfo.seat.id + 1}号(${demonName}) 选择${choiceDesc}`);
        if (allChooseLive) {
            addLog(`三名玩家都选择"生"，按规则三人全部死亡`);
        } else if (finalTargets.length > 0) {
            addLog(`选择"生"的玩家${finalTargets.map((x: number) => `${x + 1}号`).join('、')}将立即死亡`);
        } else {
            ...
            finalTargets.forEach((tid: number) => {
                killPlayer(tid, {
                    onAfterKill: (latestSeats: Seat[]) => {
                        remaining -= 1;
                        if (remaining === 0) {
                            addLog(`${nightInfo?.seat.id + 1 || ''}号(${demonName}) 处决${finalTargets.map((x: number) => `${x + 1}号`).join('、')}`);
                            setTimeout(() => {
    ...
    }, [currentModal?.type, hadesiaChoices, addLog, setHadesiaChoices, setCurrentModal, killPlayer, seats, nightInfo, isActorDisabledByPoisonOrDrunk, currentWakeIndex, wakeQueueIds, deadThisNight, continueToNextAction, getDemonDisplayName, setSelectedActionTargets, setWakeQueueIds, setCurrentWakeIndex]);

    const confirmSaintExecution = useCallback(() => {
        if (currentModal?.type !== 'SAINT_EXECUTION_CONFIRM') return;
        const { targetId } = currentModal.data;
        setCurrentModal(null);
        executePlayer(targetId, { forceExecution: true });
    }, [currentModal?.type, setCurrentModal, executePlayer]);
