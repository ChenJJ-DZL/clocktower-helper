    ...
    const statusList = useMemo(() => {
        ...
        (s.statusDetails || []).forEach(st => {
            if (st.includes('中毒') && !processed.has('poison')) {
                if (s.isDead && !st.includes('永久中毒') && !st.includes('舞蛇人中毒')) return;
                ...
                list.push({
                    ...
                    color: "red",
                    icon: "☠️",
                    duration: poisonStatus?.duration || st.match(/（(.+?)清除）/)?.[1] || '至下个黄昏'
                });
            ...
            if (st.includes('致醉') && !processed.has('drunk')) {
                ...
                list.push({
                    ...
                    color: "yellow",
                    icon: "🍺",
                    duration: drunkStatus?.duration || st.match(/（(.+?)清除）/)?.[1] || '至下个黄昏'
                });
                processed.add('drunk');
            }
        });
        ...
        if (!s.isDead && s.isPoisoned && !processed.has('poison')) {
            ...
            list.push({ key: 'poison', text: "中毒", color: "red", icon: "☠️", duration: ps?.duration || '至下个黄昏' });
            processed.add('poison');
        }
        if (!s.isDead && (s.role?.id === 'drunk' || s.isDrunk) && !processed.has('drunk')) {
            const ds = (s.statuses || []).find(st => st.effect === 'Drunk');
            list.push({ key: 'drunk', text: "醉酒", color: "yellow", icon: "🍺", duration: ds?.duration || (s.role?.id === 'drunk' ? '永久' : '至下个黄昏') });
            processed.add('drunk');
        }

        // 6. Ability spent
        if (s.hasUsedSlayerAbility) list.push({ key: `slayer_used-${s.id}`, text: "猎手已用", color: "red", icon: "🎯", duration: "永久" });
        if (s.hasUsedVirginAbility) list.push({ key: `virgin_used-${s.id}`, text: "处女失效", color: "yellow", icon: "⛔", duration: "永久" });
        if (s.hasAbilityEvenDead) list.push({ key: `ability_even_dead-${s.id}`, text: "死而有能", color: "yellow", icon: "👻", duration: "永久" });
