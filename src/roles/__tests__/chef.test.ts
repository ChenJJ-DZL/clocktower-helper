...
function seat(id: number, rid: string, rt: string, isEvil = false) {
  ...
  return {
    ...
    isEvil,
    _isEvil: () =>
      isEvil || rid === "imp" || rid === "spy" || rid === "poisoner",
  };
}
...
describe("厨师 (Chef)", () => {
  test("Wiki-JSON一致", () => {
    expect("在你的首个夜晚，你会得知场上邻座的邪恶玩家有多少对。").toBe(
  ...
  test("0对-邪恶不相邻", () => {
    const ss = [
      ...
      seat(3, "spy", "minion"),
      seat(4, "butler", "outsider"),
      seat(5, "washerwoman", "townsfolk"),
    ];
  ...
  test("2对-三名邪恶连续相邻", () => {
    const ss = [
      ...
      seat(4, "soldier", "townsfolk"),
      seat(5, "washerwoman", "townsfolk"),
    ];
    expect(calculatePairs(ss)).toBe(2);
  });
  test("陌客可能被视为邪恶", () => {
    const maySeeEvil = true;
    expect(maySeeEvil).toBe(true);
  });
  test("间谍可能不被视为邪恶", () => {
    const mayHide = true;
