...
import { buildAbilityMap, buildFullNightOrder, simulateNight } from "../../utils/invariantTesting";
...
async function runNight(snap: any, pickTargets?: any) {
  ...
  return simulateNight(snap, {
    nightCount: snap.nightCount, fullNightOrder, abilityMap, seed: 7,
    pickTargets,
  });
}
...
describe("Wave D1 国风角色官方规则", () => {
  it("穷奇：白天有外来者死亡 → 目标活尸 + 额外一名玩家死亡", async () => {
    ...
    const r = await runNight(snapshot(seats, { outsiderDiedToday: true }), (node: any) => (node.roleId === "qiongqi" ? [2] : []));
    const target = r.finalSnapshot.seats.find((s: any) => s.id === 2);
    const dead = r.finalSnapshot.seats.filter((s: any) => s.isDead).length;
    expect(target.isDead).toBe(true);
    expect(target.statusEffects.some((e: any) => e.type === "alive_dead")).toBe(true);
  ...
  it("穷奇：无外来者死亡 → 正常击杀（无活尸标记）", async () => {
    ...
    const target = r.finalSnapshot.seats.find((s: any) => s.id === 2);
    expect(target.isDead).toBe(true);
    expect(target.statusEffects.some((e: any) => e.type === "alive_dead")).toBe(false);
  });
  ...
  it("饕餮：目标角色类型互不相同 → 全部死亡", async () => {
    ...
    const dead = r.finalSnapshot.seats.filter((s: any) => s.isDead).map((s: any) => s.id).sort();
  ...
  it("梼杌：恶魔杀梼杌 → 爪牙失去能力，梼杌存活", async () => {
    ...
    expect(minion.statusEffects.some((e: any) => e.type === "lost_ability")).toBe(true);
  });
  ...
  it("梼杌：无爪牙可替死 → 梼杌死亡", async () => {
    ...
    const r = await runNight(snapshot(seats), (node: any) => (node.roleId === "imp" ? [1] : []));
    expect(r.finalSnapshot.seats.find((s: any) => s.id === 1).isDead).toBe(true);
  });

  it("鸩：限一次毒杀在场镇民角色；重复使用被拦截；不在场无影响", async () => {
    const { runFullAbilityPipeline } = await import("../../utils/middlewarePipeline");
    const abilityMap = buildAbilityMap();
    ...
    const out1 = await runFullAbilityPipeline(
      ...
      { snapshot: snapshot(seats), actionNode: node, targetIds: [], storytellerInput: { roleId: "investigator" }, meta: {}, aborted: false } as any
    );
    const victim = out1.snapshot.seats.find((s: any) => s.id === 1);
    expect(victim.isDead).toBe(true);
    expect(victim.statusEffects.some((e: any) => e.type === "poisoned")).toBe(true);

    // 2. 第二次使用 → 限次拦截
    const out2 = await runFullAbilityPipeline(
      { preCheck: ability.preCheck, calculate: ability.calculate, stateUpdate: ability.stateUpdate, postProcess: ability.postProcess },
      { snapshot: out1.snapshot, actionNode: node, targetIds: [], storytellerInput: { roleId: "chef" }, meta: {}, aborted: false } as any
    );
    ...
    const out3 = await runFullAbilityPipeline(
      { preCheck: ability.preCheck, calculate: ability.calculate, stateUpdate: ability.stateUpdate, postProcess: ability.postProcess },
      { snapshot: snapshot(seats), actionNode: node, targetIds: [], storytellerInput: { roleId: "mayor" }, meta: {}, aborted: false } as any
    );
