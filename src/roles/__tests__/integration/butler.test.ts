...
describe("管家 引擎集成测试", () => {
  test("首夜行动", async () => {
    expect((await runFullAbilityPipeline(pipe(butlerAbility), ctx(0, 1))).aborted).toBe(false);
  });
  test("非首夜行动", async () => {
    expect((await runFullAbilityPipeline(pipe(butlerAbility), ctx(0, 2))).aborted).toBe(false);
  });
