...
function s(
  ...
  rid: string,
  rt: string,
  o?: { dead?: boolean; roleSnapshot?: string; role?: any }
) {
...
function ctx(
  ...
  const marked = seats.map((s) => ({
    ...s,
    executedToday: s.id === executedSeatId,
  }));
  return {
    snapshot: {
      nightCount: nc,
