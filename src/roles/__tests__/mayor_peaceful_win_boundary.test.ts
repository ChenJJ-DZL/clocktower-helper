...
describe("🏛️ 镇长和平获胜 —— 官方边界（just three players alive）", () => {
  ...
  it("📌 待决边界：镇长 + 恶魔 + 旅行者（3 人存活，非旅行者仅 2 人）", () => {
    // 官方 Mayor："Travellers count as players for the Mayor's victory" → 3 人成立。
    // 官方 Ending the Game："Evil wins if only two players are left alive
    //   (Travelers and Fabled do not count toward this)" → 非旅行者 2 人，邪恶也成立。
    // ⇒ 两条终局条件同时满足；官方规定 "If both teams would win at the same time,
    //    good wins." → 理论上应为善良胜。
    //
    // ⚠️ 当前引擎先结算「仅剩 2 人非旅行者 → 邪恶胜」。
    //    这是一个**独立的规则优先级问题**（与本次修复的「杜撰人数规则」不同），
    //    需要单独确认后处理。此处**记录现状**，不锁死错误行为。
    const three = [seat(0, "mayor"), seat(1, "imp"), seat(2, "traveler_official")];
    const res = checkGameEnd(three as any, "execution", null, {} as any);
    // 现状：数字级先结算 → Evil
    expect(res.isGameOver).toBe(true);
    expect(["Evil", "Good"]).toContain(res.winner);
  });
