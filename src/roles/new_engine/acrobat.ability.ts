...
const calculate = async (ctx: MiddlewareContext): Promise<MiddlewareContext> => {
  ...
  const effects = target?.statusEffects ?? ctx.snapshot.statusEffects?.[targetId] ?? [];
  const isDrunkOrPoisoned = effects.some((e: any) => e.type === "drunk" || e.type === "poisoned");

  return {
    ...ctx, meta: {
      ...ctx.meta, abilityResult: {
        targetId, isDrunkOrPoisoned,
...
const stateUpdate = async (ctx: MiddlewareContext): Promise<MiddlewareContext> => {
  ...
  return {
    ...
    snapshot: {
      ...ctx.snapshot,
      _abilityResults: { ...((ctx.snapshot as any)._abilityResults ?? {}), acrobat: r },
    },
...
export const acrobatAbility = createRoleAbility({
  roleId: "acrobat", abilityId: "acrobat_detect", abilityName: "醉酒探测",
  triggerTiming: [AbilityTriggerTiming.EVERY_NIGHT],
  wakePriority: 22, firstNightOnly: false, wakePromptId: "role.acrobat.wake",
