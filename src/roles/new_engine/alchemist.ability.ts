...
const calculate = async (context: MiddlewareContext): Promise<MiddlewareContext> => {
  return { ...context, meta: { ...context.meta, abilityResult: { roleId: "alchemist" } } };
};

const postProcess = async (context: MiddlewareContext): Promise<MiddlewareContext> => {
  return { ...context, meta: { ...context.meta, abilityLog: "alchemist已完成行动" } };
};

export const alchemistAbility = createRoleAbility({
  roleId: "alchemist",
  abilityId: "alchemist_ability",
  abilityName: "alchemist能力",
  triggerTiming: [AbilityTriggerTiming.PASSIVE],
  wakePriority: 999,
  firstNightOnly: false,
  wakePromptId: "role.alchemist.wake",
  targetConfig: {
    min: 0,
    max: 0,
    allowSelf: false,
