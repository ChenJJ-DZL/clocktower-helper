...
const stateUpdate = async (ctx: MiddlewareContext): Promise<MiddlewareContext> => {
  ...
  return {
    ...
    snapshot: {
      ...ctx.snapshot,
      _abilityResults: { ...((ctx.snapshot as any)._abilityResults ?? {}), amnesiac: r },
    },
  };
};
...
const postProcess = async (ctx: MiddlewareContext): Promise<MiddlewareContext> => {
  ...
  console.log(log);
  return { ...ctx, meta: { ...ctx.meta, abilityLog: log, prompt: `唤醒${ctx.actionNode.seatId + 1}号【失忆者】，说书人已设定秘密能力。` } };
};

export const amnesiacAbility = createRoleAbility({
  roleId: "amnesiac", abilityId: "amnesiac_secret_ability", abilityName: "失忆之谜",
  triggerTiming: [AbilityTriggerTiming.FIRST_NIGHT], wakePriority: 50, firstNightOnly: true, wakePromptId: "role.amnesiac.wake",
