...
const stateUpdate = async (ctx: MiddlewareContext): Promise<MiddlewareContext> => {
  ...
  return {
    ...
    snapshot: {
      ...ctx.snapshot,
      protectedTonight: [...((ctx.snapshot as any).protectedTonight ?? []), r.targetId],
      _abilityResults: { ...((ctx.snapshot as any)._abilityResults ?? {}), angel: r },
    },
...
const postProcess = async (ctx: MiddlewareContext): Promise<MiddlewareContext> => {
  ...
  console.log(log);
  return { ...ctx, meta: { ...ctx.meta, abilityLog: log, prompt: `唤醒${ctx.actionNode.seatId + 1}号【天使】，选择一名玩家保护。` } };
};

export const angelAbility = createRoleAbility({
  roleId: "angel", abilityId: "angel_protect", abilityName: "天使庇护",
  triggerTiming: [AbilityTriggerTiming.FIRST_NIGHT], wakePriority: 35, firstNightOnly: true,
  wakePromptId: "role.angel.wake", targetConfig: { min: 1, max: 1, allowSelf: false, allowDead: false },
