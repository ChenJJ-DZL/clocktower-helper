...
const calculate = async (ctx: MiddlewareContext): Promise<MiddlewareContext> => {
  // 检测是否被恶魔杀死
  const killerRole = ctx.meta.killerRole ?? ctx.actionNode.meta?.killerRole ?? "";
  const killedByDemon = killerRole === "demon" || ctx.meta.killedByDemon === true;
  return {
    ...ctx, meta: {
      ...ctx.meta, abilityResult: { killedByDemon, bansheeAwakened: killedByDemon },
    },
  };
};
...
const stateUpdate = async (ctx: MiddlewareContext): Promise<MiddlewareContext> => {
  ...
  return {
    ...ctx, meta: { ...ctx.meta, bansheeResult: r },
    snapshot: { ...ctx.snapshot, bansheeAwakened: true, _abilityResults: { ...((ctx.snapshot as any)._abilityResults ?? {}), banshee: r } },
  };
};
...
const postProcess = async (ctx: MiddlewareContext): Promise<MiddlewareContext> => {
  ...
  console.log(log);
  return { ...ctx, meta: { ...ctx.meta, prompt: "报丧女妖被恶魔杀死，在白天的阶段宣布此事。", abilityLog: log } };
};

export const bansheeAbility = createRoleAbility({
  roleId: "banshee", abilityId: "banshee_death_awaken", abilityName: "报丧觉醒",
  triggerTiming: [AbilityTriggerTiming.ON_DEATH], wakePriority: 86, firstNightOnly: false, wakePromptId: "role.banshee.wake",
