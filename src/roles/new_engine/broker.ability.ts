...
const calculate = async (context: MiddlewareContext): Promise<MiddlewareContext> => {
  return { ...context, meta: { ...context.meta, abilityResult: { roleId: "broker" } } };
};

const postProcess = async (context: MiddlewareContext): Promise<MiddlewareContext> => {
  return { ...context, meta: { ...context.meta, abilityLog: "broker已完成行动" } };
};

export const brokerAbility = createRoleAbility({
  roleId: "broker",
  abilityId: "broker_ability",
  abilityName: "broker能力",
