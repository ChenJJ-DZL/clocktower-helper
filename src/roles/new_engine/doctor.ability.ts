...
const calculate = async (context: MiddlewareContext): Promise<MiddlewareContext> => {
  return { ...context, meta: { ...context.meta, abilityResult: { roleId: "doctor" } } };
};

const postProcess = async (context: MiddlewareContext): Promise<MiddlewareContext> => {
  return { ...context, meta: { ...context.meta, abilityLog: "doctor已完成行动" } };
};

export const doctorAbility = createRoleAbility({
  roleId: "doctor",
  abilityId: "doctor_ability",
  abilityName: "doctor能力",
