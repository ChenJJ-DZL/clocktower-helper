...
const calculate = async (context: MiddlewareContext): Promise<MiddlewareContext> => {
  return { ...context, meta: { ...context.meta, abilityResult: { roleId: "dusk" } } };
};

const postProcess = async (context: MiddlewareContext): Promise<MiddlewareContext> => {
  return { ...context, meta: { ...context.meta, abilityLog: "dusk已完成行动" } };
};

export const duskAbility = createRoleAbility({
  roleId: "dusk",
  abilityId: "dusk_ability",
  abilityName: "dusk能力",
  triggerTiming: [AbilityTriggerTiming.PASSIVE],
  wakePriority: 999,
  firstNightOnly: false,
  wakePromptId: "role.dusk.wake",
  targetConfig: {
    min: 0,
    max: 0,
    allowSelf: false,
