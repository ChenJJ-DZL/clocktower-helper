/**
 * enlightened（enlightened）新引擎技能实现
 */
...
const calculate = async (context: MiddlewareContext): Promise<MiddlewareContext> => {
  return { ...context, meta: { ...context.meta, abilityResult: { roleId: "enlightened" } } };
};

const postProcess = async (context: MiddlewareContext): Promise<MiddlewareContext> => {
  return { ...context, meta: { ...context.meta, abilityLog: "enlightened已完成行动" } };
};

export const enlightenedAbility = createRoleAbility({
  roleId: "enlightened",
  abilityId: "enlightened_ability",
  abilityName: "enlightened能力",
