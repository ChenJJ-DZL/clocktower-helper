...
const calculate = async (
  ...
  if (target && !blocked && !canJump && isTargetAlive) {
...
const postProcess = async (
  ...
  if (r?.becomesFangGu) {
    action = "→ 外来者变方古（不死亡）";
  } else if (r?.mayorSaved && r?.substituteId != null) {
    action = `攻击了 ${r.targetId + 1} 号镇长，触发替死，由 ${r.substituteId + 1} 号替代死亡`;
  } else {
    action = `击杀${r?.targetId != null ? `${r.targetId + 1}号` : ""}`;
  }
