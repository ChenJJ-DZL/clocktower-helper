  ...
  return {
    ...ctx, meta: {
      ...ctx.meta, abilityResult: {
        targetId, killed: !isOutsider,
        becomesFangGu: isOutsider, isOutsider,
      },
    },
  };
};
...
const stateUpdate = async (ctx: MiddlewareContext): Promise<MiddlewareContext> => {
  ...
  return {
    ...
    snapshot: {
      ...ctx.snapshot,
      lastKill: { demonId: ctx.actionNode.seatId, targetId: r.targetId, demonRole: "fang_gu" },
      fangGuJump: r.becomesFangGu ? r.targetId : null,
      _abilityResults: { ...((ctx.snapshot as any)._abilityResults ?? {}), fang_gu: r },
    },
...
const postProcess = async (ctx: MiddlewareContext): Promise<MiddlewareContext> => {
  const r = ctx.meta.abilityResult as any;
  const action = r?.becomesFangGu ? "→ 外来者变方古（不死亡）" : `击杀${r?.targetId != null ? r.targetId + 1 + "号" : ""}`;
  const log = `[FangGu] ${action}`;
  console.log(log);
  return { ...ctx, meta: { ...ctx.meta, prompt: `唤醒${ctx.actionNode.seatId + 1}号【方古】，选择一名玩家杀害。`, abilityLog: log } };
};

export const fang_guAbility = createRoleAbility({
  roleId: "fang_gu", abilityId: "fang_gu_kill", abilityName: "外来者猎杀",
  triggerTiming: [AbilityTriggerTiming.EVERY_NIGHT], wakePriority: 49, firstNightOnly: false,
  wakePromptId: "role.fang_gu.wake", targetConfig: { min: 1, max: 1, allowSelf: false, allowDead: false },
