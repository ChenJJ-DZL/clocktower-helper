...
const calculate = async (ctx: MiddlewareContext): Promise<MiddlewareContext> => {
  // 寻找存活的善良玩家（非邪恶类型）
  const goodCandidates = ctx.snapshot.seats.filter((s: any) =>
  ...
  const chosen = goodCandidates.length > 0
    ...
    : null;
  return { ...ctx, meta: { ...ctx.meta, abilityResult: { newFarmerId: chosen?.id ?? null, hasTransfer: chosen !== null } } };
};
...
const stateUpdate = async (ctx: MiddlewareContext): Promise<MiddlewareContext> => {
  ...
  return {
    ...ctx, meta: { ...ctx.meta, farmerResult: r },
    snapshot: { ...ctx.snapshot, _abilityResults: { ...((ctx.snapshot as any)._abilityResults ?? {}), farmer: r } },
  };
};

const postProcess = async (ctx: MiddlewareContext): Promise<MiddlewareContext> => {
  const r = ctx.meta.abilityResult as any;
  if (!r?.hasTransfer) { console.log("[Farmer] 无可用继承目标"); return ctx; }
...
export const farmerAbility = createRoleAbility({
  roleId: "farmer", abilityId: "farmer_death_transfer", abilityName: "农夫继承",
  triggerTiming: [AbilityTriggerTiming.PASSIVE], wakePriority: 0, firstNightOnly: false, wakePromptId: "role.farmer.wake",
