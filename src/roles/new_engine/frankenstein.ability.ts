/**
 * frankenstein（frankenstein）新引擎技能实现
 */
...
const calculate = async (context: MiddlewareContext): Promise<MiddlewareContext> => {
  return { ...context, meta: { ...context.meta, abilityResult: { roleId: "frankenstein" } } };
};

const postProcess = async (context: MiddlewareContext): Promise<MiddlewareContext> => {
  return { ...context, meta: { ...context.meta, abilityLog: "frankenstein已完成行动" } };
};

export const frankensteinAbility = createRoleAbility({
  roleId: "frankenstein",
  abilityId: "frankenstein_ability",
  abilityName: "frankenstein能力",
