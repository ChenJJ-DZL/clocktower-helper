...
const calculate = async (context: MiddlewareContext): Promise<MiddlewareContext> => {
  return { ...context, meta: { ...context.meta, abilityResult: { roleId: "general" } } };
};

const postProcess = async (context: MiddlewareContext): Promise<MiddlewareContext> => {
  return { ...context, meta: { ...context.meta, abilityLog: "general已完成行动" } };
};

export const generalAbility = createRoleAbility({
  roleId: "general",
  abilityId: "general_ability",
  abilityName: "general能力",
