...
const calculate = async (context: MiddlewareContext): Promise<MiddlewareContext> => {
  return { ...context, meta: { ...context.meta, abilityResult: { roleId: "hatter" } } };
};

const postProcess = async (context: MiddlewareContext): Promise<MiddlewareContext> => {
  return { ...context, meta: { ...context.meta, abilityLog: "hatter已完成行动" } };
};

export const hatterAbility = createRoleAbility({
  roleId: "hatter",
  abilityId: "hatter_ability",
  abilityName: "hatter能力",
  triggerTiming: [AbilityTriggerTiming.PASSIVE],
  wakePriority: 999,
  firstNightOnly: false,
  wakePromptId: "role.hatter.wake",
  targetConfig: {
    min: 0,
    max: 0,
    allowSelf: false,
