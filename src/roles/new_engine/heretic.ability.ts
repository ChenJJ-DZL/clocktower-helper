...
const calculate = async (context: MiddlewareContext): Promise<MiddlewareContext> => {
  return { ...context, meta: { ...context.meta, abilityResult: { roleId: "heretic" } } };
};

const postProcess = async (context: MiddlewareContext): Promise<MiddlewareContext> => {
  return { ...context, meta: { ...context.meta, abilityLog: "heretic已完成行动" } };
};

export const hereticAbility = createRoleAbility({
  roleId: "heretic",
  abilityId: "heretic_ability",
  abilityName: "heretic能力",
  triggerTiming: [AbilityTriggerTiming.PASSIVE],
  wakePriority: 999,
  firstNightOnly: false,
  wakePromptId: "role.heretic.wake",
  targetConfig: {
    min: 0,
    max: 0,
    allowSelf: false,
