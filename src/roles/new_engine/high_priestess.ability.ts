/**
 * high priestess（high_priestess）新引擎技能实现
 */
...
const calculate = async (context: MiddlewareContext): Promise<MiddlewareContext> => {
  return { ...context, meta: { ...context.meta, abilityResult: { roleId: "high_priestess" } } };
};

const postProcess = async (context: MiddlewareContext): Promise<MiddlewareContext> => {
  return { ...context, meta: { ...context.meta, abilityLog: "high priestess已完成行动" } };
};

export const high_priestessAbility = createRoleAbility({
  roleId: "high_priestess",
  abilityId: "high_priestess_ability",
  abilityName: "high priestess能力",
