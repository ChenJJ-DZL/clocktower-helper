...
const calculate = async (context: MiddlewareContext): Promise<MiddlewareContext> => {
  return { ...context, meta: { ...context.meta, abilityResult: { roleId: "historian" } } };
};

const postProcess = async (context: MiddlewareContext): Promise<MiddlewareContext> => {
  return { ...context, meta: { ...context.meta, abilityLog: "historian已完成行动" } };
};

export const historianAbility = createRoleAbility({
  roleId: "historian",
  abilityId: "historian_ability",
  abilityName: "historian能力",
