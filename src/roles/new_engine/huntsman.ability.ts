...
const calculate = async (context: MiddlewareContext): Promise<MiddlewareContext> => {
  return { ...context, meta: { ...context.meta, abilityResult: { roleId: "huntsman" } } };
};

const postProcess = async (context: MiddlewareContext): Promise<MiddlewareContext> => {
  return { ...context, meta: { ...context.meta, abilityLog: "huntsman已完成行动" } };
};

export const huntsmanAbility = createRoleAbility({
  roleId: "huntsman",
  abilityId: "huntsman_ability",
  abilityName: "huntsman能力",
