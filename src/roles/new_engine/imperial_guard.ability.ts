/**
 * imperial guard（imperial_guard）新引擎技能实现
 */
...
const calculate = async (context: MiddlewareContext): Promise<MiddlewareContext> => {
  return { ...context, meta: { ...context.meta, abilityResult: { roleId: "imperial_guard" } } };
};

const postProcess = async (context: MiddlewareContext): Promise<MiddlewareContext> => {
  return { ...context, meta: { ...context.meta, abilityLog: "imperial guard已完成行动" } };
};

export const imperial_guardAbility = createRoleAbility({
  roleId: "imperial_guard",
  abilityId: "imperial_guard_ability",
  abilityName: "imperial guard能力",
