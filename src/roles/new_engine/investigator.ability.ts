...
const calculateResult = async (
  ...
  let result: { seat1: string; seat2: string; minionName: string };
  ...
  if (!isAbilityActive) {
    ...
    // 正常情况：从预设的首夜信息中获取两个玩家，其中一个是指定爪牙
    result = meta.initialNightInfo?.investigatorInfo ?? {
      seat1: "",
      seat2: "",
      minionName: "",
    };
...
export const investigatorAbility = createRoleAbility({
  ...
  postProcess: [
    async (context) => {
      const { meta } = context;
      console.log(
        `调查员获得信息：${meta.abilityResult.seat1}号和${meta.abilityResult.seat2}号玩家中有一名是${meta.abilityResult.minionName}`
      );
      return context;
    },
  ],
