...
const calculate = async (context: MiddlewareContext): Promise<MiddlewareContext> => {
  return { ...context, meta: { ...context.meta, abilityResult: { roleId: "jinx_star" } } };
};

const postProcess = async (context: MiddlewareContext): Promise<MiddlewareContext> => {
  return { ...context, meta: { ...context.meta, abilityLog: "jinx star已完成行动" } };
};

export const jinx_starAbility = createRoleAbility({
  roleId: "jinx_star",
  abilityId: "jinx_star_ability",
  abilityName: "jinx star能力",
  triggerTiming: [AbilityTriggerTiming.PASSIVE],
  wakePriority: 999,
  firstNightOnly: false,
  wakePromptId: "role.jinx_star.wake",
  targetConfig: {
    min: 0,
    max: 0,
    allowSelf: false,
