...
const calculate = async (context: MiddlewareContext): Promise<MiddlewareContext> => {
  return { ...context, meta: { ...context.meta, abilityResult: { roleId: "legion" } } };
};

const postProcess = async (context: MiddlewareContext): Promise<MiddlewareContext> => {
  return { ...context, meta: { ...context.meta, abilityLog: "legion已完成行动" } };
};

export const legionAbility = createRoleAbility({
  roleId: "legion",
  abilityId: "legion_ability",
  abilityName: "legion能力",
  triggerTiming: [AbilityTriggerTiming.PASSIVE],
  wakePriority: 999,
  firstNightOnly: false,
  wakePromptId: "role.legion.wake",
  targetConfig: {
    min: 0,
    max: 0,
    allowSelf: false,
