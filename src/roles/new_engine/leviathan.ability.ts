...
const calculate = async (context: MiddlewareContext): Promise<MiddlewareContext> => {
  return { ...context, meta: { ...context.meta, abilityResult: { roleId: "leviathan" } } };
};

const postProcess = async (context: MiddlewareContext): Promise<MiddlewareContext> => {
  return { ...context, meta: { ...context.meta, abilityLog: "leviathan已完成行动" } };
};

export const leviathanAbility = createRoleAbility({
  roleId: "leviathan",
  abilityId: "leviathan_ability",
  abilityName: "leviathan能力",
  triggerTiming: [AbilityTriggerTiming.PASSIVE],
  wakePriority: 999,
  firstNightOnly: false,
  wakePromptId: "role.leviathan.wake",
  targetConfig: {
    min: 0,
    max: 0,
    allowSelf: false,
