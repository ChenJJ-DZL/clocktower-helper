/**
 * lycanthrope（lycanthrope）新引擎技能实现
 */
...
const calculate = async (context: MiddlewareContext): Promise<MiddlewareContext> => {
  return { ...context, meta: { ...context.meta, abilityResult: { roleId: "lycanthrope" } } };
};

const postProcess = async (context: MiddlewareContext): Promise<MiddlewareContext> => {
  return { ...context, meta: { ...context.meta, abilityLog: "lycanthrope已完成行动" } };
};

export const lycanthropeAbility = createRoleAbility({
  roleId: "lycanthrope",
  abilityId: "lycanthrope_ability",
  abilityName: "lycanthrope能力",
