...
const calculate = async (context: MiddlewareContext): Promise<MiddlewareContext> => {
  return { ...context, meta: { ...context.meta, abilityResult: { roleId: "magician" } } };
};

const postProcess = async (context: MiddlewareContext): Promise<MiddlewareContext> => {
  return { ...context, meta: { ...context.meta, abilityLog: "magician已完成行动" } };
};

export const magicianAbility = createRoleAbility({
  roleId: "magician",
  abilityId: "magician_ability",
  abilityName: "magician能力",
  triggerTiming: [AbilityTriggerTiming.PASSIVE],
  wakePriority: 999,
  firstNightOnly: false,
  wakePromptId: "role.magician.wake",
  targetConfig: {
    min: 0,
    max: 0,
    allowSelf: false,
