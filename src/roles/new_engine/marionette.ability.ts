...
const calculate = async (context: MiddlewareContext): Promise<MiddlewareContext> => {
  return { ...context, meta: { ...context.meta, abilityResult: { roleId: "marionette" } } };
};

const postProcess = async (context: MiddlewareContext): Promise<MiddlewareContext> => {
  return { ...context, meta: { ...context.meta, abilityLog: "marionette已完成行动" } };
};

export const marionetteAbility = createRoleAbility({
  roleId: "marionette",
  abilityId: "marionette_ability",
  abilityName: "marionette能力",
  triggerTiming: [AbilityTriggerTiming.PASSIVE],
  wakePriority: 999,
  firstNightOnly: false,
  wakePromptId: "role.marionette.wake",
  targetConfig: {
    min: 0,
    max: 0,
    allowSelf: false,
