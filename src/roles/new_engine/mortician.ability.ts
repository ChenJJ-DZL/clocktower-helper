...
const calculate = async (context: MiddlewareContext): Promise<MiddlewareContext> => {
  return { ...context, meta: { ...context.meta, abilityResult: { roleId: "mortician" } } };
};

const postProcess = async (context: MiddlewareContext): Promise<MiddlewareContext> => {
  return { ...context, meta: { ...context.meta, abilityLog: "mortician已完成行动" } };
};

export const morticianAbility = createRoleAbility({
  roleId: "mortician",
  abilityId: "mortician_ability",
  abilityName: "mortician能力",
