/**
 * naughty child（naughty_child）新引擎技能实现
 */
...
const calculate = async (context: MiddlewareContext): Promise<MiddlewareContext> => {
  return { ...context, meta: { ...context.meta, abilityResult: { roleId: "naughty_child" } } };
};

const postProcess = async (context: MiddlewareContext): Promise<MiddlewareContext> => {
  return { ...context, meta: { ...context.meta, abilityLog: "naughty child已完成行动" } };
};

export const naughty_childAbility = createRoleAbility({
  roleId: "naughty_child",
  abilityId: "naughty_child_ability",
  abilityName: "naughty child能力",
  triggerTiming: [AbilityTriggerTiming.PASSIVE],
  wakePriority: 999,
  firstNightOnly: false,
  wakePromptId: "role.naughty_child.wake",
  targetConfig: {
    min: 0,
    max: 0,
    allowSelf: false,
