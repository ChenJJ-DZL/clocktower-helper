/**
 * nightwatchman（nightwatchman）新引擎技能实现
 */
...
const calculate = async (context: MiddlewareContext): Promise<MiddlewareContext> => {
  return { ...context, meta: { ...context.meta, abilityResult: { roleId: "nightwatchman" } } };
};

const postProcess = async (context: MiddlewareContext): Promise<MiddlewareContext> => {
  return { ...context, meta: { ...context.meta, abilityLog: "nightwatchman已完成行动" } };
};

export const nightwatchmanAbility = createRoleAbility({
  roleId: "nightwatchman",
  abilityId: "nightwatchman_ability",
  abilityName: "nightwatchman能力",
  triggerTiming: [AbilityTriggerTiming.PASSIVE],
  wakePriority: 999,
  firstNightOnly: false,
  wakePromptId: "role.nightwatchman.wake",
  targetConfig: {
    min: 0,
    max: 0,
    allowSelf: false,
