...
const calculate = async (context: MiddlewareContext): Promise<MiddlewareContext> => {
  return { ...context, meta: { ...context.meta, abilityResult: { roleId: "outsider" } } };
};

const postProcess = async (context: MiddlewareContext): Promise<MiddlewareContext> => {
  return { ...context, meta: { ...context.meta, abilityLog: "outsider已完成行动" } };
};

export const outsiderAbility = createRoleAbility({
  roleId: "outsider",
  abilityId: "outsider_ability",
  abilityName: "outsider能力",
  triggerTiming: [AbilityTriggerTiming.PASSIVE],
  wakePriority: 999,
  firstNightOnly: false,
  wakePromptId: "role.outsider.wake",
  targetConfig: {
    min: 0,
    max: 0,
    allowSelf: false,
