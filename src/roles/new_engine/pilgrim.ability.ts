...
function calcDirection(seats: any[], selfId: number): string {
  const alive = seats.filter((s: any) => s.id !== selfId && !s.isDead && s.role && EVIL_TYPES.has(s.role.type));
...
const stateUpdate = async (ctx: MiddlewareContext): Promise<MiddlewareContext> => {
  ...
  return {
    ...ctx, meta: { ...ctx.meta, pilgrimResult: r },
    snapshot: { ...ctx.snapshot, _abilityResults: { ...((ctx.snapshot as any)._abilityResults ?? {}), pilgrim: r } },
  };
};
...
const postProcess = async (ctx: MiddlewareContext): Promise<MiddlewareContext> => {
  ...
  return {
    ...ctx, meta: {
      ...ctx.meta, prompt: `唤醒${ctx.actionNode.seatId + 1}号【修行者】，告知${r?.direction ?? "无结果"}。`,
      abilityLog: log, displayInfo: { type: "pilgrim_info", direction: r?.direction ?? "", log },
    },
  };
};

export const pilgrimAbility = createRoleAbility({
  roleId: "pilgrim", abilityId: "pilgrim_nightly_direction", abilityName: "邪恶感知",
  triggerTiming: [AbilityTriggerTiming.EVERY_NIGHT], wakePriority: 30, firstNightOnly: false, wakePromptId: "role.pilgrim.wake",
