...
const calculate = async (context: MiddlewareContext): Promise<MiddlewareContext> => {
  return { ...context, meta: { ...context.meta, abilityResult: { roleId: "preacher" } } };
};

const postProcess = async (context: MiddlewareContext): Promise<MiddlewareContext> => {
  return { ...context, meta: { ...context.meta, abilityLog: "preacher已完成行动" } };
};

export const preacherAbility = createRoleAbility({
  roleId: "preacher",
  abilityId: "preacher_ability",
  abilityName: "preacher能力",
