...
const calculate = async (context: MiddlewareContext): Promise<MiddlewareContext> => {
  return { ...context, meta: { ...context.meta, abilityResult: { roleId: "prefect" } } };
};

const postProcess = async (context: MiddlewareContext): Promise<MiddlewareContext> => {
  return { ...context, meta: { ...context.meta, abilityLog: "prefect已完成行动" } };
};

export const prefectAbility = createRoleAbility({
  roleId: "prefect",
  abilityId: "prefect_ability",
  abilityName: "prefect能力",
