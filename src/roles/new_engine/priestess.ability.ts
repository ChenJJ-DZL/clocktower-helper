...
const calculate = async (ctx: MiddlewareContext): Promise<MiddlewareContext> => {
  const candidates = ctx.snapshot.seats.filter((s: any) => s.id !== ctx.actionNode.seatId);
  ...
  return {
    ...ctx, meta: { ...ctx.meta, abilityResult: { targetId: target?.id ?? candidates[0]?.id ?? 0 } }
  };
};
...
const stateUpdate = async (ctx: MiddlewareContext): Promise<MiddlewareContext> => {
  ...
  return {
    ...ctx, meta: { ...ctx.meta, priestessResult: r },
    snapshot: { ...ctx.snapshot, _abilityResults: { ...((ctx.snapshot as any)._abilityResults ?? {}), priestess: r } },
  };
};
...
const postProcess = async (ctx: MiddlewareContext): Promise<MiddlewareContext> => {
  ...
  return {
    ...ctx, meta: {
      ...ctx.meta, prompt: `唤醒${ctx.actionNode.seatId + 1}号【女祭司】，告知${r.targetId + 1}号是最值得交流的玩家。`,
      abilityLog: log, displayInfo: { type: "priestess_info", player: r.targetId, log },
    },
  };
};

export const priestessAbility = createRoleAbility({
  roleId: "priestess", abilityId: "priestess_nightly_ability", abilityName: "神谕指引",
  triggerTiming: [AbilityTriggerTiming.EVERY_NIGHT], wakePriority: 30, firstNightOnly: false, wakePromptId: "role.priestess.wake",
