...
const stateUpdate = async (ctx: MiddlewareContext): Promise<MiddlewareContext> => {
  ...
  return {
    ...
    snapshot: {
      ...
      demonBlockedTonight: true,
      _abilityResults: { ...((ctx.snapshot as any)._abilityResults ?? {}), princess: r },
    },
  };
};
...
const postProcess = async (ctx: MiddlewareContext): Promise<MiddlewareContext> => {
  ...
  console.log(log);
  return { ...ctx, meta: { ...ctx.meta, abilityLog: log, prompt: "公主展示了身份。此次处决不导致死亡，今晚恶魔无法造成死亡。" } };
};

export const princessAbility = createRoleAbility({
  roleId: "princess", abilityId: "princess_nomination_save", abilityName: "公主豁免",
  triggerTiming: [AbilityTriggerTiming.PASSIVE], wakePriority: 0, firstNightOnly: false, wakePromptId: "role.princess.wake",
