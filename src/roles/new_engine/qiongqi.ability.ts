...
const calculate = async (context: MiddlewareContext): Promise<MiddlewareContext> => {
  return { ...context, meta: { ...context.meta, abilityResult: { roleId: "qiongqi" } } };
};

const postProcess = async (context: MiddlewareContext): Promise<MiddlewareContext> => {
  return { ...context, meta: { ...context.meta, abilityLog: "qiongqi已完成行动" } };
};

export const qiongqiAbility = createRoleAbility({
  roleId: "qiongqi",
  abilityId: "qiongqi_ability",
  abilityName: "qiongqi能力",
