...
const calculate = async (context: MiddlewareContext): Promise<MiddlewareContext> => {
  return { ...context, meta: { ...context.meta, abilityResult: { roleId: "scholar" } } };
};

const postProcess = async (context: MiddlewareContext): Promise<MiddlewareContext> => {
  return { ...context, meta: { ...context.meta, abilityLog: "scholar已完成行动" } };
};

export const scholarAbility = createRoleAbility({
  roleId: "scholar",
  abilityId: "scholar_ability",
  abilityName: "scholar能力",
  triggerTiming: [AbilityTriggerTiming.PASSIVE],
  wakePriority: 999,
  firstNightOnly: false,
  wakePromptId: "role.scholar.wake",
  targetConfig: {
    min: 0,
    max: 0,
    allowSelf: false,
