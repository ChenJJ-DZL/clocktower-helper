...
const calculate = async (context: MiddlewareContext): Promise<MiddlewareContext> => {
  return { ...context, meta: { ...context.meta, abilityResult: { roleId: "scribe" } } };
};

const postProcess = async (context: MiddlewareContext): Promise<MiddlewareContext> => {
  return { ...context, meta: { ...context.meta, abilityLog: "scribe已完成行动" } };
};

export const scribeAbility = createRoleAbility({
  roleId: "scribe",
  abilityId: "scribe_ability",
  abilityName: "scribe能力",
  triggerTiming: [AbilityTriggerTiming.PASSIVE],
  wakePriority: 999,
  firstNightOnly: false,
  wakePromptId: "role.scribe.wake",
  targetConfig: {
    min: 0,
    max: 0,
    allowSelf: false,
