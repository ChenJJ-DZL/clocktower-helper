...
const calculate = async (context: MiddlewareContext): Promise<MiddlewareContext> => {
  return { ...context, meta: { ...context.meta, abilityResult: { roleId: "shugenja" } } };
};

const postProcess = async (context: MiddlewareContext): Promise<MiddlewareContext> => {
  return { ...context, meta: { ...context.meta, abilityLog: "shugenja已完成行动" } };
};

export const shugenjaAbility = createRoleAbility({
  roleId: "shugenja",
  abilityId: "shugenja_ability",
  abilityName: "shugenja能力",
