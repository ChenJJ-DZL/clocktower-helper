...
const calculate = async (context: MiddlewareContext): Promise<MiddlewareContext> => {
  return { ...context, meta: { ...context.meta, abilityResult: { roleId: "singer" } } };
};

const postProcess = async (context: MiddlewareContext): Promise<MiddlewareContext> => {
  return { ...context, meta: { ...context.meta, abilityLog: "singer已完成行动" } };
};

export const singerAbility = createRoleAbility({
  roleId: "singer",
  abilityId: "singer_ability",
  abilityName: "singer能力",
  triggerTiming: [AbilityTriggerTiming.PASSIVE],
  wakePriority: 999,
  firstNightOnly: false,
  wakePromptId: "role.singer.wake",
  targetConfig: {
    min: 0,
    max: 0,
    allowSelf: false,
