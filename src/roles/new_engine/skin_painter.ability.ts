/**
 * skin painter（skin_painter）新引擎技能实现
 */
...
const calculate = async (context: MiddlewareContext): Promise<MiddlewareContext> => {
  return { ...context, meta: { ...context.meta, abilityResult: { roleId: "skin_painter" } } };
};

const postProcess = async (context: MiddlewareContext): Promise<MiddlewareContext> => {
  return { ...context, meta: { ...context.meta, abilityLog: "skin painter已完成行动" } };
};

export const skin_painterAbility = createRoleAbility({
  roleId: "skin_painter",
  abilityId: "skin_painter_ability",
  abilityName: "skin painter能力",
