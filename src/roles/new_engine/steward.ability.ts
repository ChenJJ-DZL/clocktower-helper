...
const calculate = async (context: MiddlewareContext): Promise<MiddlewareContext> => {
  return { ...context, meta: { ...context.meta, abilityResult: { roleId: "steward" } } };
};

const postProcess = async (context: MiddlewareContext): Promise<MiddlewareContext> => {
  return { ...context, meta: { ...context.meta, abilityLog: "steward已完成行动" } };
};

export const stewardAbility = createRoleAbility({
  roleId: "steward",
  abilityId: "steward_ability",
  abilityName: "steward能力",
  triggerTiming: [AbilityTriggerTiming.PASSIVE],
  wakePriority: 999,
  firstNightOnly: false,
  wakePromptId: "role.steward.wake",
  targetConfig: {
    min: 0,
    max: 0,
    allowSelf: false,
