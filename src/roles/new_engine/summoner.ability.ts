...
const calculate = async (context: MiddlewareContext): Promise<MiddlewareContext> => {
  return { ...context, meta: { ...context.meta, abilityResult: { roleId: "summoner" } } };
};

const postProcess = async (context: MiddlewareContext): Promise<MiddlewareContext> => {
  return { ...context, meta: { ...context.meta, abilityLog: "summoner已完成行动" } };
};

export const summonerAbility = createRoleAbility({
  roleId: "summoner",
  abilityId: "summoner_ability",
  abilityName: "summoner能力",
