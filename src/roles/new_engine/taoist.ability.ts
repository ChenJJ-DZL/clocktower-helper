...
const calculate = async (context: MiddlewareContext): Promise<MiddlewareContext> => {
  return { ...context, meta: { ...context.meta, abilityResult: { roleId: "taoist" } } };
};

const postProcess = async (context: MiddlewareContext): Promise<MiddlewareContext> => {
  return { ...context, meta: { ...context.meta, abilityLog: "taoist已完成行动" } };
};

export const taoistAbility = createRoleAbility({
  roleId: "taoist",
  abilityId: "taoist_ability",
  abilityName: "taoist能力",
  triggerTiming: [AbilityTriggerTiming.PASSIVE],
  wakePriority: 999,
  firstNightOnly: false,
  wakePromptId: "role.taoist.wake",
  targetConfig: {
    min: 0,
    max: 0,
    allowSelf: false,
