...
const calculate = async (context: MiddlewareContext): Promise<MiddlewareContext> => {
  return { ...context, meta: { ...context.meta, abilityResult: { roleId: "taotie" } } };
};

const postProcess = async (context: MiddlewareContext): Promise<MiddlewareContext> => {
  return { ...context, meta: { ...context.meta, abilityLog: "taotie已完成行动" } };
};

export const taotieAbility = createRoleAbility({
  roleId: "taotie",
  abilityId: "taotie_ability",
  abilityName: "taotie能力",
