...
const calculate = async (context: MiddlewareContext): Promise<MiddlewareContext> => {
  return { ...context, meta: { ...context.meta, abilityResult: { roleId: "vizier" } } };
};

const postProcess = async (context: MiddlewareContext): Promise<MiddlewareContext> => {
  return { ...context, meta: { ...context.meta, abilityLog: "维齐尔已完成行动" } };
};

export const vizierAbility = createRoleAbility({
  roleId: "vizier",
  abilityId: "vizier_ability",
  abilityName: "维齐尔能力",
  triggerTiming: [AbilityTriggerTiming.PASSIVE],
  wakePriority: 999,
  firstNightOnly: false,
  wakePromptId: "role.vizier.wake",
  targetConfig: {
    min: 0,
    max: 0,
    allowSelf: false,
