...
const calculate = async (context: MiddlewareContext): Promise<MiddlewareContext> => {
  return { ...context, meta: { ...context.meta, abilityResult: { roleId: "wizard" } } };
};

const postProcess = async (context: MiddlewareContext): Promise<MiddlewareContext> => {
  return { ...context, meta: { ...context.meta, abilityLog: "wizard已完成行动" } };
};

export const wizardAbility = createRoleAbility({
  roleId: "wizard",
  abilityId: "wizard_ability",
  abilityName: "wizard能力",
  triggerTiming: [AbilityTriggerTiming.PASSIVE],
  wakePriority: 999,
  firstNightOnly: false,
  wakePromptId: "role.wizard.wake",
  targetConfig: {
    min: 0,
    max: 0,
    allowSelf: false,
