...
const calculate = async (context: MiddlewareContext): Promise<MiddlewareContext> => {
  return { ...context, meta: { ...context.meta, abilityResult: { roleId: "wraith" } } };
};

const postProcess = async (context: MiddlewareContext): Promise<MiddlewareContext> => {
  return { ...context, meta: { ...context.meta, abilityLog: "wraith已完成行动" } };
};

export const wraithAbility = createRoleAbility({
  roleId: "wraith",
  abilityId: "wraith_ability",
  abilityName: "wraith能力",
