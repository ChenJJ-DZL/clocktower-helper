...
const calculate = async (context: MiddlewareContext): Promise<MiddlewareContext> => {
  return { ...context, meta: { ...context.meta, abilityResult: { roleId: "xaan" } } };
};

const postProcess = async (context: MiddlewareContext): Promise<MiddlewareContext> => {
  return { ...context, meta: { ...context.meta, abilityLog: "xaan已完成行动" } };
};

export const xaanAbility = createRoleAbility({
  roleId: "xaan",
  abilityId: "xaan_ability",
  abilityName: "xaan能力",
  triggerTiming: [AbilityTriggerTiming.PASSIVE],
  wakePriority: 999,
  firstNightOnly: false,
  wakePromptId: "role.xaan.wake",
  targetConfig: {
    min: 0,
    max: 0,
    allowSelf: false,
