/**
 * yaggababble（yaggababble）新引擎技能实现
 */
...
const calculate = async (context: MiddlewareContext): Promise<MiddlewareContext> => {
  return { ...context, meta: { ...context.meta, abilityResult: { roleId: "yaggababble" } } };
};

const postProcess = async (context: MiddlewareContext): Promise<MiddlewareContext> => {
  return { ...context, meta: { ...context.meta, abilityLog: "yaggababble已完成行动" } };
};

export const yaggababbleAbility = createRoleAbility({
  roleId: "yaggababble",
  abilityId: "yaggababble_ability",
  abilityName: "yaggababble能力",
