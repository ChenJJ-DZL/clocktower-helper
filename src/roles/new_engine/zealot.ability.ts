...
const calculate = async (context: MiddlewareContext): Promise<MiddlewareContext> => {
  return { ...context, meta: { ...context.meta, abilityResult: { roleId: "zealot" } } };
};

const postProcess = async (context: MiddlewareContext): Promise<MiddlewareContext> => {
  return { ...context, meta: { ...context.meta, abilityLog: "zealot已完成行动" } };
};

export const zealotAbility = createRoleAbility({
  roleId: "zealot",
  abilityId: "zealot_ability",
  abilityName: "zealot能力",
  triggerTiming: [AbilityTriggerTiming.PASSIVE],
  wakePriority: 999,
  firstNightOnly: false,
  wakePromptId: "role.zealot.wake",
  targetConfig: {
    min: 0,
    max: 0,
    allowSelf: false,
