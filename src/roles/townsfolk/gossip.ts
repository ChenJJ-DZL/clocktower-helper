...
- 角色类型：镇民`,
    ...
    handler: (context: DayActionContext): DayActionResult => {
      return {
        updates: [
          {
            id: context.selfId,
            statusDetails: [...(context.seats[context.selfId].statusDetails || []), '今日已造谣']
          }
        ],
        logs: {
  ...
  night: {
    ...
    dialog: (playerSeatId: number) => ({
      ...
      instruction: "如果该玩家今日发表了正确的传闻，说书人应选择一名玩家额外死亡。",
    ...
    handler: (context: NightActionContext): NightActionResult | null => {
      ...
      return {
        updates: [
          {
            id: selfId,
            statusDetails: (selfSeat?.statusDetails || []).filter(d => d !== '今日已造谣')
          }
        ],
        modal: {
          ...
          data: {
            ...
            roleId: 'gossip',
            roleName: '造谣者',
            description: "如果该玩家今日造谣为【真】，请选择一名玩家死亡（额外死亡）。否则直接取消。",
            targetCount: 1,
