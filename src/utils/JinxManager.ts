...
import { getRegistration, RegistrationResult } from './gameRules';
...
export class JinxManager {
    ...
    static interceptInspection(
        ...
        const jinx = getJinxesForCharacter(viewerId).find(j => j.character2 === targetId);
    ...
    static canPerformAction(
        ...
        if (actorId === 'vizier' && target) {
            ...
            const fearmonger = allSeats.find(s => s.role?.id === 'fearmonger' && !s.isDead);
            if (fearmonger && fearmonger.statusDetails?.some(d => d.includes('恐惧之灵选中') && d.includes(target.id.toString()))) {
                return { allowed: false, reason: '相克规则：维齐尔无法对恐惧之灵选中的目标使用能力' };
            }
        }
