 ...
 */
export function getExecutedByDay(
  dayCount: number = 0
): DeathRecord | undefined {
  const deathRecords = getDeathRecords(0);
 ...
 * 获取所有被处决的玩家记录
 */
export function getAllExecutedPlayers(): DeathRecord[] {
  return getDeathRecords(0).filter((r) => r.deathReason === "executed");
}
 ...
 * @param endTime 结束时间戳，默认为当前时间
 */
export function getNominationRecords(
  startTime: number,
  endTime: number = Date.now()
): NominationRecord[] {
  ...
  for (const snapshot of nominationSnapshots) {
    ...
    if (nominateLog) {
      ...
      // 优先使用 nominationRecords 字段
      const nominatorId =
        snapshot.nominationRecords?.nominators.size > 0
          ? Array.from(snapshot.nominationRecords.nominators)[0]
          : snapshot.nominatorPlayers.size > 0
            ? Array.from(snapshot.nominatorPlayers)[0]
            : 0;
      const nominatedId =
        snapshot.nominationRecords?.nominees.size > 0
          ? Array.from(snapshot.nominationRecords.nominees)[0]
 ...
 */
export function getPlayerStateAtNight(
  seatId: number,
  nightCount: number
): Seat | undefined {
  const snapshots = historySnapshotManager.getSnapshotsByNightCount(nightCount);
 ...
 */
export function getPlayerStateAtDay(
  seatId: number,
  dayCount: number
): Seat | undefined {
 ...
 * @param endTime 结束时间
 */
export function isPlayerDiedInPeriod(
  seatId: number,
  startTime: number,
  endTime: number
): boolean {
  const stateAtStart = getPlayerStateAtTime(seatId, startTime);
  const stateAtEnd = getPlayerStateAtTime(seatId, endTime);
 ...
 * @param seatId 玩家ID
 */
export function getPlayerAlignmentChanges(
  seatId: number
 ...
 * @param seatId 玩家ID
 */
export function getPlayerRoleChanges(seatId: number): Array<{
  timestamp: number;
  fromRole: string;
  toRole: string;
  fromType: string;
...
export function countExecutions(
  ...
  return getAllExecutedPlayers().filter(
 ...
 * @param count 次数
 */
export function getLastExecutions(count: number): DeathRecord[] {
  return getAllExecutedPlayers().slice(-count);
}
 ...
 * @param endTime 结束时间
 */
export function countPlayerNominations(
  seatId: number,
  asNominator: boolean = true,
  startTime: number = 0,
  endTime: number = Date.now()
): number {
  const records = getNominationRecords(startTime, endTime);
 ...
 * @param endTime 结束时间
 */
export function hasRoleExistedInPeriod(
  roleId: string,
  startTime: number = 0,
  endTime: number = Date.now()
): boolean {
