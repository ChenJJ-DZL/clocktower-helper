...
test.describe("高级游戏模拟器", () => {
  test("随机化对局测试", async ({ page }) => {
    ...
    if (drunkVisible) {
      await page.evaluate(() => {
        ...
        for (let i = 0; i < btns.length; i++) {
          if ((btns[i].textContent || "").includes("设置酒鬼身份")) { btns[i].click(); return; }
        }
      });
      ...
      await page.evaluate(() => {
        ...
        for (let i = 0; i < b2.length; i++) {
          ...
          if (t && t !== "✕" && !t.includes("确认选择")) { b2[i].click(); return; }
        }
      });
      ...
      await page.evaluate(() => {
        ...
        for (let i = 0; i < b3.length; i++) {
          if ((b3[i].textContent || "").includes("确认选择") && !b3[i].disabled) { b3[i].click(); return; }
        }
      });
      await page.waitForTimeout(1500);
    }
    ...
    await page.evaluate(() => {
      const btns = document.querySelectorAll("button");
      for (let i = 0; i < btns.length; i++) {
        if ((btns[i].textContent || "").includes("入夜") && !btns[i].disabled) { btns[i].click(); return; }
      }
    });
    await page.waitForTimeout(3000);
    ...
    for (let i = 0; i < 10; i++) {
      ...
      if (await m.first().isVisible().catch(() => false)) {
        await m.getByRole("button").first().click(); continue;
      }
      // 确认按钮
      const c = page.getByRole("button", { name: /确认|下一步/ });
      if (await c.isVisible().catch(() => false) && await c.isEnabled().catch(() => false)) {
        await c.click(); continue;
      }
      // 目标选择
      const seats = page.locator('[class*="cursor-pointer"]');
      if ((await seats.count().catch(() => 0)) > 0) {
        const idx = Math.floor(Math.random() * await seats.count().catch(() => 0));
        await seats.nth(idx).click();
        await page.waitForTimeout(2000);
        if (!(await c.isEnabled().catch(() => false))) {
          const idx2 = (idx + 1) % (await seats.count().catch(() => 0));
          await seats.nth(idx2).click().catch(() => {});
          await page.waitForTimeout(2000);
        }
    ...
    await page.evaluate(() => {
      const btns = document.querySelectorAll("button");
      for (let i = 0; i < btns.length; i++) {
        if ((btns[i].textContent || "").trim() === "确认" && !btns[i].disabled) { btns[i].click(); return; }
      }
    });
    await page.waitForTimeout(1500);
    await page.evaluate(() => {
      const btns = document.querySelectorAll("button");
      for (let i = 0; i < btns.length; i++) {
        if ((btns[i].textContent || "").includes("黄昏") && !btns[i].disabled) { btns[i].click(); return; }
      }
    });
    await page.waitForTimeout(1500);
    ...
    const hasNight = await page.evaluate(() => {
      const btns = document.querySelectorAll("button");
      for (let i = 0; i < btns.length; i++) {
        if ((btns[i].textContent || "").includes("入夜") && !btns[i].disabled) return true;
      }
      return false;
    });
