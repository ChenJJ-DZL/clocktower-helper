...
interface ExpRole { name: string; id: string; }

const EXP_BATCHES: ExpRole[][] = [
  // Batch A: 镇民类
  [{name:"食人族",id:"cannibal"},{name:"失忆者",id:"amnesiac"},{name:"无神论者",id:"atheist"},
   {name:"工程师",id:"engineer"},{name:"矿工",id:"miner"},{name:"游侠",id:"ranger"}],
  // Batch B: 镇民类
  [{name:"杂技演员",id:"acrobat"},{name:"骑士",id:"knight"},{name:"贵族",id:"noble"},
   {name:"朝圣者",id:"pilgrim"},{name:"女祭司",id:"priestess"},{name:"唱诗男孩",id:"choir_boy"}],
  // Batch C: 镇民+外来者
  [{name:"公主",id:"princess"},{name:"农夫",id:"farmer"},{name:"半兽人",id:"half_ogre"},
   {name:"报丧女妖",id:"banshee"},{name:"星象师",id:"astrologer"},{name:"气球驾驶员",id:"balloonist"}],
  // Batch D: 特殊+爪牙+恶魔
  [{name:"市长",id:"mayor"},{name:"解谜大师",id:"puzzlemaster"},{name:"告密者",id:"snitch"},
   {name:"提线木偶",id:"marionette"},{name:"维齐尔",id:"vizier"},{name:"巫师",id:"wizard"}],
];

async function setupCustomScript(page: any, roles: ExpRole[], batchName: string) {
...
test.describe("第2批: 实验角色(24)", () => {
  for (let b = 0; b < EXP_BATCHES.length; b++) {
    ...
    test(`批次${batchName}: 自定义剧本创建+启动`, async ({ page }) => {
      ...
      await assign("洗衣妇", 1); await assign("厨师", 2);
      await assign("图书管理员", 3); await assign("小恶魔", 4);
