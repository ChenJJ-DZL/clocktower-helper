...
test.describe("Comprehensive E2E Game Simulation", () => {
  ...
  const seatedRoles: SeatedPlayer[] = [];
  ...
  test.beforeAll(async ({ browser }) => {
    ...
    page.on("console", (msg) => {
      if (
        !msg.text().includes("Immersive Translate") &&
  ...
  test("should run a full game simulation", async () => {
    ...
    if (
      !bodyText.includes("暗流涌动") &&
      !bodyText.includes("Trouble Brewing")
    ) {
      log(
        `[Debug] 页面上没找到剧本文字. 当前页面文字片段: ${bodyText.slice(0, 200)}...`
      );
      // 也可以打印关键按钮
    }
    ...
    log(
      `已选角色 (${selectedRoles.length}名): ${selectedRoles.map((id) => getRoleById(id)?.name || id).join(", ")}`
    );
    ...
    // 处理酒鬼伪装身份选择（如果存在）
    await handleDrunkCharadeIfPresent(page, {
      log: (phase: string, msg: string) => log(`[${phase}] ${msg}`),
    } as any);
    ...
    for (let day = 1; day <= 10; day++) {
      ...
      if (await gameEndElement.isVisible({ timeout: 1000 })) {
        const winnerText =
          (await gameEndElement.locator("[data-winner-team]").textContent()) ||
          "未知";
        log(`\n--- 游戏在第 ${day} 天结束 ---`);
        log(`胜利阵营: ${winnerText}`);
        break;
      ...
      if (await executionModal.isVisible()) {
        log("关闭处决结果报告。");
        await page
          .locator('div[aria-modal="true"] button:has-text("确认")')
          .click();
      }
    }
  });
  ...
  async function handleNightPhase(day: number) {
    ...
    for (let i = 0; i < getLivingPlayers().length + 2; i++) {
      // Safety break
      const nightPhaseActive = await page
        .locator('div[data-testid="night-phase-active"]')
        .isVisible();
      ...
      const seatIndex = parseInt(
        (await activePlayerElement.getAttribute("data-active-player-seat")) ||
          "-1",
        10
      );
      ...
      if (needsTarget) {
        const targets = shuffleArray(
          getLivingPlayers().filter((p) => p.seatIndex !== seatIndex)
        );
        const targetCount = 1;
        for (let j = 0; j < targetCount; j++) {
          if (targets[j]) {
            ...
            log(
              ` -> 目标: ${targets[j].player} (${getRoleById(targets[j].roleId)?.name})`
            );
          }
        }
      }
  ...
  async function handleDayPhase(day: number) {
    ...
    if (day > 1 && aliveIndexes.length >= 3) {
      ...
      const nominatorRole =
        seatedRoles.find((r) => r.seatIndex === nominator)?.player ||
        `${nominator}号`;
      const nomineeRole =
        seatedRoles.find((r) => r.seatIndex === nominee)?.player ||
        `${nominee}号`;

      log(` -> 尝试提名: ${nominatorRole} 提名 ${nomineeRole}`);
      await page
        .locator(`.seat-node[data-seat-id="${nominator}"]`)
        .click({ force: true });
      await sleep(300);
      await page
        .locator(`.seat-node[data-seat-id="${nominee}"]`)
        .click({ force: true });
      await sleep(300);
            ...
            for (const voter of aliveIndexes) {
              if (Math.random() > 0.4) {
                const voterBtn = voteModal
                  .locator("button")
                  .filter({ hasText: new RegExp(`^${voter + 1}号`) })
                  .first();
                if (
                  (await voterBtn.isVisible()) &&
                  !(await voterBtn.isDisabled())
                ) {
    ...
    if (await executeButton.isVisible({ timeout: 2000 })) {
      ...
      const toNightButton = page.getByRole("button", {
        name: /入夜 \(下一回合\) 🌙/,
      });
