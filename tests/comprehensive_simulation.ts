...
const pickPreset = () =>
  TB_PRESETS[Math.floor(Math.random() * TB_PRESETS.length)];

const filterRoles = (type: string) =>
  roles.filter(
    (r: Role) => (r.script === "暗流涌动" || !r.script) && r.type === type
  );
...
class GameSimulation {
  ...
  setCurrentModal = (modal: any) => {
    ...
    if (modal) {
      const currentPhaseLog =
        this.gameLog.phases[this.gameLog.phases.length - 1];
      if (currentPhaseLog) {
  ...
  killPlayer = (targetId: number, _options: any = {}) => {
    this.setSeats((prev: Seat[]) =>
      prev.map((s) => (s.id === targetId ? { ...s, isDead: true } : s))
    );
  ...
  setup() {
    ...
    const seatArray: Seat[] = Array.from(
      { length: total },
      (_, id) =>
        ({
          id,
    ...
    this.gameLog.setup.seats = this.seats.map((s) => ({
      id: s.id + 1,
...
async function run() {
  ...
  for (let i = 0; i < SIMULATION_COUNT; i++) {
    ...
    if (i % 10 === 0)
      console.log(`Completed simulation ${i}/${SIMULATION_COUNT}`);
  }
