...
test("debug body text", async ({ page }) => {
  ...
  if (idx1 >= 0) console.log(`游戏结束 at ${idx1}: "${body.substring(Math.max(0,idx1-30), idx1+50)}"`);
  if (idx2 >= 0) console.log(`胜利 at ${idx2}: "${body.substring(Math.max(0,idx2-30), idx2+50)}"`);
  if (idx3 >= 0) console.log(`获胜 at ${idx3}: "${body.substring(Math.max(0,idx3-30), idx3+50)}"`);
