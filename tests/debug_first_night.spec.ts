...
test.describe("首夜流程调试", () => {
  ...
  async function collectPageState(page: Page, stepName: string): Promise<void> {
    const bodyText = await page.evaluate(
      () => document.body?.innerText?.substring(0, 2000) || "EMPTY"
    );
    ...
    const hasModal = await page.evaluate(() => {
      ...
      return {
        count: modals.length,
        texts: Array.from(modals).map((m) =>
          m.textContent?.trim()?.substring(0, 200)
        ),
      };
    });
    ...
    const gameState = await page.evaluate(() => {
      ...
      const nightInfoEl = document.querySelector(
        '[class*="wake"], [class*="night-info"]'
      );
      return {
        phaseText: phaseEl?.textContent?.trim() || "N/A",
        nightInfoText:
          nightInfoEl?.textContent?.trim()?.substring(0, 500) || "N/A",
  ...
  async function clickAndCollect(
    page: Page,
    stepName: string,
    selectorOrFn: string | (() => Promise<void>)
  ): Promise<boolean> {
  ...
  test("完整首夜流程 - 暗流涌动", async ({ page }) => {
    ...
    page.on("console", (msg) => {
      ...
      // 特别关注关键日志
      if (
        text.includes("continueToNextAction") ||
        text.includes("handleNight") ||
        text.includes("executeViaNewEngine") ||
        text.includes("handlerResult") ||
        text.includes("preCheck") ||
        text.includes("blocked") ||
        text.includes("系统步骤") ||
        text.includes("无 handler") ||
        text.includes("nightInfo") ||
    ...
    // === Step 3: 快速测试 ===
    const quickTestBtn = page
      .locator("text=快速测试")
      .or(page.locator('button:has-text("快速测试")'))
      .first();
    ...
    for (let waitSec = 0; waitSec < 10; waitSec++) {
      ...
      // 检查是否已经到了夜晚阶段
      if (
        currentPhase.includes("首夜") ||
        currentPhase.includes("唤醒") ||
    ...
    for (let step = 1; step <= 25; step++) {
      ...
      const clickResult = await page.evaluate(() => {
        ...
        return {
          success: false,
          reason: `NO_CONFIRM_BUTTON found. Visible: [${visibleButtons.join(", ")}]`,
        };
      });
      ...
      if (!clickResult.success) {
        ...
        if (stuckCount >= MAX_STUCK) {
          ...
          await page.screenshot({
            path: `/workspace/debug_e2e_stuck_step${step}.png`,
          });
          break;
        }
      } else {
      ...
      if (step <= 10 || step % 5 === 0 || !clickResult.success) {
        ...
        await page.screenshot({
          path: `/workspace/debug_e2e_night_step${step}.png`,
        });
      }

      // 检测是否到达白天
      if (
        currentText.includes("白天") ||
        currentText.includes("黄昏") ||
        currentText.includes("平安夜") ||
        currentText.includes("死亡报告") ||
        currentText.includes("讨论") ||
        currentText.includes("提名")
      ) {
    ...
    // 输出所有关键控制台日志摘要
    console.log("\n\n===== 关键日志摘要 =====");
    const keyLogs = consoleLogs.filter(
      (l) =>
