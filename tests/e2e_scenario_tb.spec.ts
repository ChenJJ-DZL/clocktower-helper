...
test('暗流涌动(Trouble Brewing) 5人局完整流程测试', async ({ page }) => {
  ...
  if (seatElements.length === 0) {
    // 查找包含数字 1-15 的圆形元素
    for (let i = 1; i <= 15; i++) {
      const seatWithNumber = await page.locator(`div:has-text("${i}"):has-text("空")`).first().catch(() => null);
      if (seatWithNumber) {
        const parent = await seatWithNumber.locator('..').first().catch(() => null);
        if (parent) seatElements.push(parent);
      }
    }
