...
test.describe
  .skip("Expansion Roles Specific Tests", () => {
    let page: Page;
    ...
    test("Moonchild revenge kill logic", async () => {
      ...
      const grandmotherBtn = page.getByRole("button", {
        name: /祖母|GRANDMOTHER/i,
      });
      ...
      // Wait for Day Phase
      await expect(page.locator("text=/第\\s*1\\s*天|Day\\s*1/")).toBeVisible({
        timeout: 15000,
      });
      ...
      // Bulk vote
      const _voteCountText = await page
      ...
      const voters = await page
        .locator('div[role="dialog"] button:has-text("号")')
        .all();
      ...
      await expect(page.locator("text=/月之子.*陪葬/")).toBeVisible({
        timeout: 10000,
      });
    ...
    test("Devil's Advocate protection prevents execution death", async () => {
      ...
      await expect(page.locator("text=/处决未生效|活了下来/")).toBeVisible({
        timeout: 5000,
      });
    ...
    test("No-Dashii proximity poisoning", async () => {
      ...
      const _seat0 = page.locator(".seat-node").nth(0);
    ...
    test("Grandmother grandchild assignment and death trigger", async () => {
      ...
      await skipNightActions(page, logger, async () => {
        ...
        logger.log(
          "测试",
          `[NightAction] cnt: ${cnt}, text: ${instructionText}`
        );
...
async function skipNightActions(
  page: Page,
  logger: StorytellerLogger,
  onTargetNeeded?: () => Promise<boolean>
) {
  ...
  for (let i = 0; i < 100; i++) {
    try {
      ...
      const nomadBtn = page.getByRole("button", { name: "发起提名" });
      const dayIndicator = page.getByRole("button", {
        name: "进入黄昏处决阶段",
      });

      const isDay =
        (await nomadBtn.isVisible({ timeout: 100 }).catch(() => false)) ||
      ...
      if (await nightReport.isVisible({ timeout: 100 }).catch(() => false)) {
        ...
        const reportConfirm = page.locator(
          'div[aria-modal="true"] button:has-text("确认")'
        );
        if (
          await reportConfirm.isVisible({ timeout: 100 }).catch(() => false)
        ) {
      ...
      if (
        await stSelectHeading.isVisible({ timeout: 100 }).catch(() => false)
      ) {
        logger.log("测试", "探测到说书人选择弹窗");
        // Click a player button inside the modal to select a target
        const playerBtns = page
          .locator('button:has-text("号")')
        ...
        if (
          (await stConfirmBtn.isVisible({ timeout: 200 }).catch(() => false)) &&
      ...
      if (
        await checkConfirmBtn.isVisible({ timeout: 100 }).catch(() => false)
      ) {
      ...
      const confirmBtn = page
        .locator(
          'button:has-text("确认"), button:has-text("下一步"), button:has-text("Confirm"), button:has-text("说书人选好了"), button:has-text("我记住了"), button:has-text("Got it")'
        )
        .first();
