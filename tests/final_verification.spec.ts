...
test.describe("Final Verification of Day Abilities (Artist & Savant)", () => {
  ...
  test("Targeted Verification: Artist and Savant Abilities", async ({
    ...
    page.on("console", (msg) => {
      const text = msg.text();
      if (
        text.includes("[NightLogic]") ||
        text.includes("[DayAction]") ||
        text.includes("[Modal]")
      ) {
    ...
    logger.log(
      "角色分配",
      `手动分配角色: ${targetRoles.map((r) => r.name).join(", ")}`
    );
    ...
    for (let i = 0; i < targetRoles.length; i++) {
      ...
      logger.log(
        "角色分配",
        `正在为 ${seatIndex + 1} 号位分配 ${role.name}...`
      );
      ...
      try {
        ...
        await page.screenshot({
          path: path.join(__dirname, `fail_assign_${role.id}.png`),
        });
        throw e;
      }
    }
    ...
    while (nightSafety < 30) {
      ...
      if (
        (await dawnBtn.isVisible()) ||
        (await dawnText.isVisible()) ||
        (await nightReportConfirmBtn.isVisible())
      ) {
        ...
        if (await dawnBtn.isVisible()) {
          ...
          await dawnBtn
            .click()
            .catch((e) => logger.log("错误", `点击天亮按钮失败: ${e.message}`));
      ...
      if (await nextActionBtn.isVisible()) {
        const modalTitle = await page
          .locator(".modal-header, .text-xl.font-bold")
          .first()
          .innerText()
          .catch(() => "弹窗");
    ...
    await page
      .waitForSelector("text=艺术家提问", { timeout: 10000 })
      .catch(() => logger.log("警告", "未通过文本找到艺术家弹窗"));
    ...
    await page
      .waitForSelector("text=博学者信息", { timeout: 10000 })
      .catch(() => logger.log("警告", "未通过文本找到博学者弹窗"));
