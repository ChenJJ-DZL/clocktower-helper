...
let playerRoles: Record<number, string> = {};
...
test.describe("Unified Storyteller Simulation", () => {
    ...
    test("Full Game Simulation with Storyteller Perspective", async ({ page }) => {
        ...
        while (!loaded && retryCount < 3) {
            try {
                ...
                await page.waitForSelector(`[data-testid=script-card-${TEST_SCRIPT}]`, { state: 'visible', timeout: 60000 });
                loaded = true;
            } catch (e) {
                logger.log("剧本选择", `⚠️ 页面加载超时或按钮未显示 (尝试 ${retryCount + 1}/3)，正在刷新页面...`);
                await page.reload();
        ...
        await page.waitForSelector('.active-character-instruction', { state: 'visible', timeout: 15000 });
        ...
        while (!gameOver && day < 10) {
            ...
            for (let i = 0; i < 10; i++) {
                ...
                if (await modal.isVisible()) {
                    ...
                    const closeBtn = modal.locator('button').filter({ hasText: /确认|我知道了|关闭|天亮了/ }).first();
            ...
            const enterDuskBtn = page.getByRole("button", { name: /进入黄昏处决阶段/ });
            try {
                ...
                logger.log(`第 ${day} 天`, "⚠️ 未找到‘进入黄昏’按钮，可能已在黄昏阶段或被弹窗阻挡");
            }
            ...
            try {
                ...
                logger.log(`第 ${day} 天`, "⚠️ 提名进入超时或组件未渲染，尝试直接跳过...");
            }
            ...
            const enterNightBtn = page.getByRole("button", { name: /入夜 \(下一回合\) 🌙/ });
...
async function assignRolesAndLog(page: Page, logger: StorytellerLogger, rolesToAssign: string[], playerCount: number) {
    ...
    await page.waitForSelector('.seat-node', { state: 'visible', timeout: 10000 });
    ...
    for (let i = 0; i < rolesToAssign.length; i++) {
        ...
        const roleBtn = page.locator('button').filter({ hasText: roleName }).first();

        try {
            await roleBtn.scrollIntoViewIfNeeded({ timeout: 5000 });
            await roleBtn.click({ force: true, timeout: 5000 });
        } catch (e) {
            await page.screenshot({ path: path.join(__dirname, `error_role_${roleName}.png`) });
            logger.log("错误", `无法选中角色 [${roleName}]。可能原因：角色不在当前剧本中或被遮挡。`);
            throw e;
        }
        ...
        try {
            ...
            await page.screenshot({ path: path.join(__dirname, `error_seat_${seatIdx}.png`) });
            logger.log("错误", `无法点击 ${seatIdx + 1} 号位。`);
            throw e;
        }

        logger.log("落座阶段", `${i + 1}号角色的玩家选择了 ${roleName}，成功落座在 ${seatIdx + 1} 号位。`);
...
async function processNightActions(page: Page, logger: StorytellerLogger, dayOrNight: number) {
    ...
    while (hasMoreActions) {
        ...
        const currentActive = await page.locator('.active-character-instruction').innerText({ timeout: 10000 }).catch(() => null);

        // Check if we are still in night phase (e.g. if we see "天亮了" button, night is over)
        const isDawn = await page.getByRole("button", { name: /天亮了/ }).isVisible();
        ...
        const activeSeatIdx = Object.keys(playerRoles).find(k => playerRoles[parseInt(k)] === activeRoleName);

        const activeSeatLabel = activeSeatIdx !== undefined ? `${parseInt(activeSeatIdx) + 1}号位` : "未知位置";

        logger.log(phaseLabel, `[角色行动] ${activeRoleName} (${activeSeatLabel}) 正在执行行动...`);
        ...
        if (await modal.isVisible()) {
            ...
            if (modalText.includes("筑梦师信息")) {
                const revealedRoles = await modal.locator('.text-2xl.font-bold').allInnerTexts();
                logger.log(phaseLabel, `└─ [筑梦师信息] 获得信息: ${revealedRoles.join(' 和 ')}`);
            } else if (modalText.includes("夜晚死亡报告")) {
                logger.log(phaseLabel, `└─ [死亡报告] ${modalText.split('\n')[1] || ""}`);
            }

            logger.log(phaseLabel, "检测到活动弹窗，尝试处理...");
            const modalConfirmBtn = modal.locator('button').filter({ hasText: /确认|下一步/ }).first();
        ...
        if (isTargetSelectionVisible) {
            const minTargets = await targetHeader.first().getAttribute('data-min').then(v => parseInt(v || '1'));

            const aliveIndexes = await getAlivePlayerIndexes(page);

            // Log min targets and current selection if possible
            const currentSelected = await page.locator('.GameConsole button.bg-blue-600').count();
            ...
            if (currentSelected < minTargets && aliveIndexes.length > 0) {
                ...
                for (const targetIdx of targets) {
                    ...
                    logger.log(phaseLabel, `└─ 选择目标: ${targetIdx + 1}号位 (${targetRole})`);

                    const consoleSeat = page.locator('.GameConsole button').filter({ hasText: new RegExp(`^${targetIdx + 1}($|\\D)`) }).first();
                    if (await consoleSeat.isVisible()) {
                        await consoleSeat.click({ force: false }).catch(() => consoleSeat.click({ force: true }));
                    } else {
                        const seat = page.locator(`.seat-node[data-seat-id="${targetIdx}"]`);
                        await seat.click({ force: true });
                    }
                    await page.waitForTimeout(100);
                }
            } else if (currentSelected >= minTargets) {
                logger.log(phaseLabel, `└─ 目标已由系统通过 (当前已选: ${currentSelected}/${minTargets})`);
            }
        }
        ...
        if (await nextBtn.isVisible()) {
            let nextBtnDisabled = await nextBtn.isDisabled();
            if (!nextBtnDisabled) {
                await nextBtn.click();
                await page.waitForTimeout(500);
            } else {
                logger.log(phaseLabel, "确认按钮不可用，检查是否有弹窗需要处理...");
                const modalConfirmBtn = page.locator('div[role="dialog"] button').filter({ hasText: /确认|下一步/ }).first();
                if (await modalConfirmBtn.isVisible()) {
                    await modalConfirmBtn.click();
                    await page.waitForTimeout(500);
                }
...
async function performRandomDayActions(page: Page, logger: StorytellerLogger, day: number) {
    // Random chance for someone to use a day ability if available
    if (Math.random() > 0.4) { // Increased frequency to test new roles
        // Look for buttons that start with "使用 " (case insensitive or containing)
        const dayActionButtons = await page.locator('button').filter({ hasText: /^使用 / }).all();
        if (dayActionButtons.length > 0) {
            ...
            if (await modal.isVisible()) {
                ...
                const confirmBtn = modal.locator('button').filter({ hasText: /确认|记录|是|否/ }).first();
...
async function performNominations(page: Page, logger: StorytellerLogger, day: number) {
    ...
    if (await dawnModal.isVisible()) {
        ...
        if (text.includes("夜晚报告") || text.includes("黎明") || text.includes("死亡")) {
            logger.log(phaseLabel, "检测到剩余弹窗，正在关闭...");
            const closeBtn = dawnModal.locator('button').filter({ hasText: /确认|我知道了|关闭/ }).first();
            if (await closeBtn.isVisible()) {
                await closeBtn.click();
                await page.waitForTimeout(500);
            }
    ...
    for (let i = 0; i < nomCount; i++) {
        ...
        logger.log(phaseLabel, `[提名行动] ${nominator + 1}号位(${nomRole}) 提名了 ${nominee + 1}号位(${targetRole})`);

        try {
            // Use specialized selectors for seats in Dusk phase if needed, but .seat-node is usually fine
            const nominatorSeat = page.locator(`.seat-node[data-seat-id="${nominator}"]`);
    ...
    if (await endNomBtn.isVisible()) {
        ...
        const modal = page.locator('div[role="dialog"]');
        if (await modal.isVisible()) {
            const confirmBtn = modal.locator('button').filter({ hasText: /确认|确定/ }).first();
            if (await confirmBtn.isVisible()) {
                await confirmBtn.click();
            }
        }
...
async function simulateVoting(page: Page, logger: StorytellerLogger, day: number, nominee: number) {
    ...
    try {
        ...
        for (const voterIdx of voters) {
            if (Math.random() > 0.4) {
                const voterBtn = voteModal.locator('button').filter({ hasText: new RegExp(`^${voterIdx + 1}号`) }).first();
        ...
        logger.log(phaseLabel, `└─ 投票玩家: ${actualVoters.map(v => `${v + 1}号(${playerRoles[v] || "未知"})`).join(", ")}`);
