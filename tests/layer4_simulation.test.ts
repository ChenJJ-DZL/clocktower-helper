...
describe("Layer 4: 全剧本批量仿真", () => {
  it("批量运行 " +
    SCRIPTS.length * GAMES_PER_SCRIPT +
    " 场对局并生成覆盖度报告", async () => {
    const allReports = [];
    ...
    for (const script of SCRIPTS) {
      for (let g = 0; g < GAMES_PER_SCRIPT; g++) {
        ...
        for (const t of report.triggers) {
          ...
          if (e) {
            e.triggerCount++;
            if (t.success) e.successCount++;
            if (t.corrupted) e.corruptedCount++;
            if (t.error) e.errorCount++;
          } else {
            coverageMap.set(t.roleId, {
              roleId: t.roleId,
              triggerCount: 1,
              successCount: t.success ? 1 : 0,
              corruptedCount: t.corrupted ? 1 : 0,
              errorCount: t.error ? 1 : 0,
            });
          }
        }
      }
    ...
    console.log(
      "剧本数: " + SCRIPTS.length + "  每剧本局数: " + GAMES_PER_SCRIPT
    );
    ...
    console.log(
      "触发角色: " +
        ...
        allAssigned.size +
        " (" +
        ((triggeredRoles / allAssigned.size) * 100).toFixed(1) +
        "%)"
    );
    ...
    for (const r of allReports) {
      if (!scriptStats[r.script])
        scriptStats[r.script] = {
          games: 0,
          triggers: 0,
          errors: 0,
          crashes: 0,
    ...
    for (const [s, d] of Object.entries(scriptStats)) {
      console.log(
        "  " +
          ...
          " 错误:" +
          d.errors +
          (d.crashes > 0 ? " 崩溃:" + d.crashes : "")
      );
    }

    // 错误详情
    const allErrorDetails = allReports.flatMap((r, i) =>
      r.errors.map((e) => "  [游戏" + (i + 1) + "/" + r.script + "] " + e)
    );
    ...
    for (const [id, e] of [...coverageMap.entries()].sort((a, b) => {
      const rateA =
        a[1].triggerCount > 0 ? a[1].successCount / a[1].triggerCount : 0;
      const rateB =
        b[1].triggerCount > 0 ? b[1].successCount / b[1].triggerCount : 0;
      return rateA - rateB;
    })) {
      const rate =
        e.triggerCount > 0
          ? ((e.successCount / e.triggerCount) * 100).toFixed(0) + "%"
          : "N/A";
    ...
    // 未触发角色
    const neverTriggered = [...allAssigned].filter(
      (id) => !coverageMap.has(id)
    );
    if (neverTriggered.length > 0) {
      const maybeNight = neverTriggered.filter(
        (id) => !passiveOrDay.includes(id)
      );
      const knownPassive = neverTriggered.filter((id) =>
        passiveOrDay.includes(id)
      );
      console.log("\n--- 未触发角色 (" + neverTriggered.length + "个) ---");
      if (maybeNight.length > 0)
        console.log("  可能需要night能力排查: " + maybeNight.join(", "));
      if (knownPassive.length > 0)
        console.log("  被动/日间角色(预期): " + knownPassive.join(", "));
    }
