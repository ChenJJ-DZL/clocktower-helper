...
export async function skipToDay(page: Page) {
  for (let i = 0; i < 100; i++) {
    ...
    if (await duskBtn.isVisible({ timeout: 300 }).catch(() => false)) {
      for (let m = 0; m < 10; m++) {
        const modal = page
          .locator("[data-modal-key], .fixed.inset-0.z-50")
          .first();
    ...
    if (
      await page
        .locator("h2:has-text('夜晚报告'), text=昨晚, text=平安夜")
        .first()
    ...
    const nextBtn = page
      ...
      .first();
    if (
      (await nextBtn.isVisible({ timeout: 200 }).catch(() => false)) &&
      (await nextBtn.isEnabled().catch(() => false))
    ) {
      try {
        await nextBtn.click({ timeout: 500, force: true });
    ...
    if (await modal.isVisible({ timeout: 100 }).catch(() => false)) {
      const modalKey =
        (await modal.getAttribute("data-modal-key").catch(() => "")) ?? "";
      ...
      if (modalKey.includes("占卜师") || modalText.includes("占卜师")) {
        const btn = modal
          .locator("button:has-text('是'),button:has-text('确认')")
          .first();
      ...
      if (
        !handled &&
        (modalText.includes("选择角色") ||
          modalText.includes("变成") ||
          modalText.includes("展示") ||
          modalText.includes("疯狂"))
      ) {
      ...
      if (handled) {
        ...
        for (let w = 0; w < 20; w++) {
          ...
          // 确认按钮可用→点击
          if (
            (await nextBtn.isVisible({ timeout: 100 }).catch(() => false)) &&
            (await nextBtn.isEnabled().catch(() => false))
          ) {
            try {
              await nextBtn.click({ timeout: 500, force: true });
