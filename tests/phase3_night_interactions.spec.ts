...
const NIGHT_ROLES: NightRoleTest[] = [
  {
    roleName: "占卜师", script: "暗流涌动", demon: "小恶魔",
    townsfolk: ["洗衣妇","厨师"], minion: "投毒者",
    targets: 2, hasExtraStep: false, roleId: "fortune_teller",
  },
  {
    roleName: "女裁缝", script: "梦殒春宵", demon: "涡流",
    townsfolk: ["筑梦师","数学家"], minion: "麻脸巫婆",
    targets: 2, hasExtraStep: false, roleId: "seamstress",
  },
  {
    roleName: "舞蛇人", script: "梦殒春宵", demon: "涡流",
    townsfolk: ["筑梦师","数学家"], minion: "麻脸巫婆",
    targets: 1, hasExtraStep: false, roleId: "snake_charmer",
  },
];
...
async function handleNightWithRole(
  ...
  for (let i = 0; i < 30; i++) {
    // 白天检测
    if (await page.locator("button:has-text('进入黄昏处决阶段')")
    ...
    if (await report.first().isVisible({ timeout: 200 }).catch(() => false)) {
    ...
    // 检测目标角色行动
    const isTargetRole = await page.locator(`text=${roleName}`).first()
    ...
    if (isTargetRole && !handled) {
      ...
      if (hasExtraStep) {
        ...
        const choice = page.locator("button:not([disabled]):not(:has-text('#'))").first();
      ...
      // 确认
      const nextBtn = page.locator("button").filter({ hasText: /确认.*下一步/ }).first();
    ...
    if (await nextBtn.isVisible({ timeout: 200 }).catch(() => false)) {
      if (await nextBtn.isEnabled().catch(() => false)) {
        await nextBtn.click(); await page.waitForTimeout(400); continue;
      }
      const hashBtn = page.locator("button:has-text('#')").first();
      if (await hashBtn.isVisible({ timeout: 200 }).catch(() => false)) {
        await hashBtn.click(); await page.waitForTimeout(300); continue;
      }
    }
    await page.waitForTimeout(500);
  }
...
test.describe("Phase 3 - 夜间交互", () => {
  for (const role of NIGHT_ROLES) {
    test(`${role.roleName} - 夜间选目标确认 (${role.script})`, async ({ page }) => {
      ...
      const handled = await handleNightWithRole(page, role.roleName, role.targets, role.hasExtraStep);
