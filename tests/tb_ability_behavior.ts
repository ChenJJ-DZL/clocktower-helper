...
function ok(role: string, check: string, detail = "") {
  results.push({ role, check, pass: true, detail });
}
function fail(role: string, check: string, detail: string) {
  results.push({ role, check, pass: false, detail });
}

function triggersByRole(
  reports: GameReport[],
  roleId: string
): TriggerRecord[] {
  return reports.flatMap((r) =>
    r.triggers.filter((t) => t.roleId === roleId && t.timing === "night")
  );
}
...
function testFirstNightOnlyRoles(reports: GameReport[], allRoles: Set<string>) {
  ...
  for (const { id, name, jsonFn } of fnRoles) {
    if (!allRoles.has(id)) {
      fail(name, "覆盖", "50局中未出现此角色");
      continue;
    }
    ok(
      name,
      "覆盖",
      `出现在${reports.filter((r) => r.seedsRoleIds.includes(id)).length}局中`
    );
    ...
    if (nonFirst.length > 0) {
      fail(
        name,
        "首夜专属",
        `在非首夜触发了${nonFirst.length}次 (R${nonFirst.map((t) => t.round).join(",R")})`
      );
    } else {
      ok(name, "首夜专属", "仅在首夜触发");
    }
    ...
    for (const { id: cid, name: cname } of fnRoles) {
      if (cid === id) continue;
      const crossTriggers = triggersByRole(reports, cid).filter(
        (t) => t.round !== 1
      );
      if (crossTriggers.length > 0) {
        fail(
          cname,
          "首夜专属",
          `被${name}检测到非首夜触发: ${crossTriggers.length}次`
        );
      }
    }
  }
  ...
  if (allRoles.has("poisoner")) {
    ...
    if (withTargets.length < poiTriggers.length * 0.5) {
      fail(
        "投毒者",
        "目标选择",
        `${withTargets.length}/${poiTriggers.length}次有目标`
      );
    } else {
      ok(
        "投毒者",
...
function testOtherNightOnlyRoles(reports: GameReport[], allRoles: Set<string>) {
  ...
  for (const { id, name } of onRoles) {
    ...
    if (otherNights.length === 0) {
      ...
      if (id === "ravenkeeper")
        ok(name, "条件触发", "守鸦人仅死亡时触发，属正常");
      else if (id === "scarlet_woman")
        ok(name, "条件触发", "红唇女郎仅恶魔死亡时触发，属正常");
      else if (id === "undertaker")
        ok(name, "条件触发", "送葬者仅处决后触发，属正常");
  ...
  for (const { id, name } of passiveRoles) {
    ...
    if (nightTriggers.length > 0) {
      fail(
        name,
        "夜晚禁止",
        `JSON标记无法行动但夜晚触发了${nightTriggers.length}次`
      );
    } else {
      ok(name, "夜晚禁止", "夜晚未触发 ✅");
    }
...
function testSpecialRules(reports: GameReport[], allRoles: Set<string>) {
  ...
  if (allRoles.has("imp")) {
    ...
    if (impFirst.length > 0)
      fail("小恶魔", "首夜禁止", `首夜触发${impFirst.length}次`);
  ...
  if (allRoles.has("monk")) {
    ...
    if (monkFirst.length > 0)
      fail("僧侣", "首夜禁止", `首夜触发${monkFirst.length}次`);
  ...
  if (allRoles.has("undertaker")) {
    const utFirst = reports.flatMap((r) =>
      r.triggers.filter(
        (t) =>
          t.roleId === "undertaker" && t.timing === "night" && t.round === 1
      )
    );
    if (utFirst.length > 0)
      fail("送葬者", "首夜禁止", `首夜触发${utFirst.length}次`);
...
function printReport() {
  ...
  for (const [role, checks] of Object.entries(byRole)) {
    ...
    console.log(
      `  ${status} ${role}: ${checks.map((c) => c.check).join(", ")}`
    );
  }
  console.log();
