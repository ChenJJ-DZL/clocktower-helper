...
async function runAll() {
  for (let g = 0; g < GAMES; g++) {
    ...
    for (let round = 1; round <= report.totalRounds; round++) {
      const nightTriggers = report.triggers.filter(
      ...
      for (let i = 1; i < nightTriggers.length; i++) {
        ...
        if (pr && cr) {
          ...
          if (pOrd !== null && cOrd !== null && pOrd > cOrd) {
            violations.push(
              `[顺序错误] 第${round}轮(${isFirst ? "首夜" : "其他夜"}): ${pr.name}#${pOrd} 晚于 ${cr.name}#${cOrd}`
            );
          }
        }
      }
