...
function findRule(roleId: string) {
  return (
    rules.get(roleId.toLowerCase().replace(/[^a-z]/g, "")) || rules.get(roleId)
  );
}
  ...
  for (let g = 0; g < GAMES; g++) {
      ...
      for (let i = 1; i < nt.length; i++) {
        ...
        if (pr && cr) {
          ...
          if (pOrd !== null && cOrd !== null && pOrd > cOrd) {
            violations.push(
              `[顺序错误] 第${round}轮(${isFirst ? "首夜" : "其他夜"}): ${pr.name}#${pOrd} -> ${cr.name}#${cOrd} (顺序颠倒)`
            );
          }
        }
      }
    ...
    for (const t of report.triggers) {
      ...
      if (
        ...
        violations.push(
          `[首夜专属] ${r.name} 在第${t.round}轮被唤醒(应仅首夜)`
        );
      }
      if (
        r.firstNightOrder === null &&
    ...
    const ftFirstNight = report.triggers.filter(
      (t) =>
    ...
    if (roleIds.includes("baron")) {
      ...
      const outsiderIds = roleIds.filter(
        (id) => findRule(id)?.type === "outsider"
      );
      const townsfolkIds = roleIds.filter(
        (id) => findRule(id)?.type === "townsfolk"
      );
    ...
    // === E. 崩溃/错误 ===
    if (report.crashed)
      violations.push(`[崩溃] 游戏${g}: ${report.crashMessage}`);
