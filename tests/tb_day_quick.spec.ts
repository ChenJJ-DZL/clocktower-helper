...
test("日间能力UI组件可用性验证", async ({ page }) => {
  ...
  const startEnabled = await startBtn.isEnabled({ timeout: 5000 }).catch(() => false);
  ...
  for (const role of roles) {
    ...
    console.log(`  ${hasRole ? "✅" : "⚠️"} ${role}: ${hasRole ? "已分配" : "未找到"}`);
  }
...
test("贞洁者提名流程组件检查", async ({ page }) => {
  ...
  const exists = await virginBtn.isVisible({ timeout: 5000 }).catch(() => false);
