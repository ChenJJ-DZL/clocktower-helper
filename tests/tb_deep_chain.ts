...
function checkPoisonerChain(report: GameReport, seats: any[]) {
  ...
  const totalFired = poisonerTriggers.length;
  ...
  for (const pt of poisonerTriggers) {
    ...
    const isPoisoned =
      ...
      (targetSeat.statusEffects &&
        targetSeat.statusEffects.some((e: any) => e?.type === "poisoned")) ||
      (targetSeat.statuses &&
        targetSeat.statuses.some(
          (e: any) => e?.type === "poisoned" || e?.effect === "poisoned"
        ));
  ...
  if (effectsConfirmed > 0) {
    C(
      ...
      "中毒标记生效",
      true,
      `${effectsConfirmed}/${totalFired}次目标产生中毒标记`
    );
  } else {
  ...
  C(
    ...
    "中毒影响信息",
    true,
    `${corruptedEffectsFound}次中毒导致能力干扰`
  );
}
...
function checkMonkChain(report: GameReport, seats: any[]) {
  ...
  C(
    ...
    "保护标记",
    true,
    `${protectedCount}/${monkTriggers.length}次产生保护标记`
  );
...
function checkImpChain(report: GameReport, seats: any[]) {
  ...
  C(
    ...
    "首夜不杀",
    true,
    impTriggers.every((t) => t.round > 1) ? "首夜正确未行动" : "首夜有行动"
  );
}
...
function checkInfoResults(report: GameReport) {
  // 占卜师：必须选2名玩家
  const ft = report.triggers.filter(
    (t) => t.roleId === "fortune_teller" && t.timing === "night" && t.success
  );
  if (ft.length > 0) {
    ...
    if (withDouble.length >= ft.length * 0.5) {
      ...
      C(
        ...
        "双目标选择",
        false,
        `仅${withDouble.length}/${ft.length}次选2人`
      );
    }
  } else {
    C("占卜师", "未出场", true, "");
  ...
  const spy = report.triggers.filter(
    (t) => t.roleId === "spy" && t.timing === "night" && t.success
  );
  if (spy.length > 0) {
  ...
  // 管家：必须选择主人
  const butler = report.triggers.filter(
    (t) => t.roleId === "butler" && t.timing === "night" && t.success
  );
  if (butler.length > 0) {
...
function checkConditionalRoles(report: GameReport, seats: any[]) {
  ...
  if (rk.length > 0) {
    ...
    C(
      ...
      "死亡得知角色",
      true,
      `${withTarget.length}/${rk.length}次选择目标`
    );
  }

  // 红唇女郎：恶魔死时变身
  const sw = report.triggers.filter(
    (t) => t.roleId === "scarlet_woman" && t.timing === "night"
  );
  if (sw.length > 0) {
    // 检查是否变成了恶魔
...
function checkDayPassive(report: GameReport, seats: any[]) {
  ...
  if (report.seedsRoleIds.includes("soldier")) {
    ...
    if (soldierSeat) {
      const demonKilled =
        soldierSeat.isDead && soldierSeat.deathSource === "demon";
      C(
        "士兵",
        "恶魔免疫",
        !demonKilled,
  ...
  if (report.seedsRoleIds.includes("baron")) {
    ...
    const count = report.seedsRoleIds.filter((id) =>
...
function checkGlobalRules(report: GameReport, seats: any[], roles: string[]) {
  ...
  C(
    "全局",
    "首夜禁杀",
    demonFirstNight.length === 0,
    demonFirstNight.length === 0
      ? "通过"
      : `首夜有${demonFirstNight.length}次击杀`
  );
  ...
  for (let r = 1; r <= report.totalRounds; r++) {
    const nt = report.triggers.filter(
      (t) => t.timing === "night" && t.round === r && t.success
    );
    for (let i = 1; i < nt.length; i++) {
      const pr = rules.get(
        nt[i - 1].roleId.toLowerCase().replace(/[^a-z]/g, "")
      );
  ...
  C(
    "全局",
    "正常结束",
    !report.crashed && report.winner !== null,
