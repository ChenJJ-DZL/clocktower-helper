...
console.log("\nTest Case 1: Evil Twin blocks Good win (Demon dead)");
{
  ...
  assert(
    result.winner !== "Good",
    "Good should NOT win if Evil Twin is active"
  );
  ...
  assert(
    ...
    "Game should continue (Twin active, >2 players)"
  );
}
...
console.log("\nTest Case 2: Demon Dead -> Good Wins (Standard)");
{
  ...
  const result = checkGameEnd(seats, "execution", 2);
  assert(
    result.isGameOver === true && result.winner === "Good",
    "Good should win if Demon is dead"
  );
}
...
console.log("\nTest Case 3: Mastermind delays Good Win");
{
  ...
  assert(
    result.isGameOver === false,
    "Game should continue if Mastermind is active"
  );
}
...
console.log("\nTest Case 4: Saint Executed -> Evil Wins");
{
  ...
  assert(
    result.isGameOver === true && result.winner === "Evil",
    "Evil should win if Saint executed"
  );
}
...
console.log("\nTest Case 5: 2 Players Alive -> Evil Wins");
{
  ...
  assert(
    result.isGameOver === true && result.winner === "Evil",
    "Evil should win if only 2 alive"
  );
}
...
console.log("\nTest Case 6: Mayor Wins");
{
  ...
  assert(
    result.isGameOver === true && result.winner === "Good",
    "Mayor should win if 3 alive and no execution"
  );
}
...
console.log("\nTest Case 7: Zombuul (Dead but effective) -> Game Continues");
{
  ...
  assert(
    result.isGameOver === false,
    "Game should continue if Zombuul is dead but effective"
  );
}
...
function crreateZombuulSeats() {
  return [
    ...
    } as any,
    {
      id: 1,
      isDead: false,
      role: { id: "townsfolk", type: "townsfolk", side: "good" },
    } as any,
    {
      id: 2,
      isDead: false,
